...
class BaseArangoService:
    ...
    async def save_to_people_collection(self, entity_id: str, email: str) -> bool:
        ...
        try:
            ...
            # Query only for count, not full documents, for existence check.
            query = "RETURN LENGTH(FOR doc IN people FILTER doc.email == @email RETURN 1)"
            count_cursor = self.db.aql.execute(query, bind_vars={"email": email})
            exists_count = next(count_cursor, 0)  # Should always yield 0 or 1
            if not exists_count:
                self.logger.info(
                    "➕ Entity does not exist, saving to people collection"
                )
                # Reuse the collection object to minimize lookup overhead
                people_collection = self.db.collection(CollectionNames.PEOPLE.value)
                people_collection.insert(
