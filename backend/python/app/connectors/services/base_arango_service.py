...
class BaseArangoService:
    ...
    async def reindex_failed_connector_records(self, user_id: str, org_id: str, connector: str, origin: str) -> Dict:
        ...
        try:
            ...
            try:
                # Generate timestamp once for all payload values
    ...
    async def _check_connector_reindex_permissions(self, user_key: str, org_id: str, connector: str, origin: str) -> Dict:
        ...
        try:
            ...
            # Prepare bind variables outside hot-path loop to avoid repeated dictionary creation
            bind_vars = {
                "user_key": user_key,
                "org_id": org_id,
                "connector": connector,
    ...
    async def get_user_by_user_id(self, user_id: str) -> Optional[Dict]:
        """Get user by user ID"""
        try:
            # Avoid string formatting the query in hot path;
            # Use bind variables for CollectionNames.USERS.value
            query = (
                "FOR user IN @@users "
                "FILTER user.userId == @user_id "
                "RETURN user"
