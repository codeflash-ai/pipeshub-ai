...
class BaseArangoService:
    ...
    async def get_user_by_user_id(self, user_id: str) -> Optional[Dict]:
        """Get user by user ID"""
        try:
            # Use precomputed AQL string to avoid repeat formatting cost
            cursor = self.db.aql.execute(_USER_BY_USER_ID_QUERY, bind_vars={"user_id": user_id})

            # Collect result using next only if we know cursor is always an iterator;
            # This avoids unnecessary variable assignments inside the try block.
            return next(cursor, None)
        except Exception as e:
