...
class BaseArangoService:
    ...
    async def get_key_by_external_message_id(
        ...
        try:
            query = (
                f"FOR doc IN {CollectionNames.RECORDS.value} FILTER doc.externalRecordId == @external_message_id RETURN doc._key"
            )
            db = transaction if transaction else self.db

            # Use next with __iter__ directly for cursor performance if available, and avoid repeated logging logic.
            cursor = db.aql.execute(query, bind_vars={"external_message_id": external_message_id})
            result = next(cursor, None)

            if result is not None:
                # Only log if a debug or trace loglevel is enabled, typically left for configuration, assume cost otherwise
                # Retain info for success as required by instruction and profile, but condense logic for logging
                self.logger.info(
