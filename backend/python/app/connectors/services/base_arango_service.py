...
class BaseArangoService:
    ...
    async def get_document(self, document_key: str, collection: str) -> Optional[Dict]:
        ...
        try:
            ...
            FOR doc IN @@collection
                ...
                RETURN doc
            """
            # Await the ArangoDB async execute method if available
    ...
    async def batch_upsert_nodes(
        ...
        try:
            ...
            cursor = await db.aql.execute(batch_query, bind_vars=bind_vars)
            results = list(cursor)
    ...
    async def store_page_token(
        ...
        try:
            self.logger.info(
                "🚀 Storing page token: Channel: %s, Resource: %s, User Email: %s, Token: %s, Expiration: %s",
                channel_id,
                resource_id,
                user_email,
                token,
            ...
            # Check/create collection only when necessary (fast path most runs)
            if not hasattr(self, "_page_tokens_collection_exists"):
                self._page_tokens_collection_exists = self.db.has_collection(
                    CollectionNames.PAGE_TOKENS.value
                )
            if not self._page_tokens_collection_exists:
                self.db.create_collection(CollectionNames.PAGE_TOKENS.value)

                self._page_tokens_collection_exists = True
            ...
            # Direct execution, no need to materialize full list
            next(
                self.db.aql.execute(
                    query,
                    bind_vars={
    ...
    async def get_page_token_db(
        ...
        try:
            self.logger.info(
                "🔍 Getting page token for: Channel: %s, Resource: %s, User Email: %s",
                channel_id,
                resource_id,
                user_email,
            )
            ...
            filters_gen = (
                expr
                for expr in (
                    "token.channelId == @channel_id"
                    if channel_id is not None
                    else None,
                    "token.resourceId == @resource_id"
                    if resource_id is not None
                    else None,
                    "token.userEmail == @user_email"
                    if user_email is not None
                    else None,
                )
                if expr is not None
            )
            filters = list(filters_gen)
            ...
            result_one = next(self.db.aql.execute(query, bind_vars=bind_vars), None)
