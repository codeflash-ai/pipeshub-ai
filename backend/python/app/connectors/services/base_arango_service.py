...
class BaseArangoService:
    ...
    async def get_app_by_name(self, name: str, transaction: Optional[TransactionDatabase] = None) -> Optional[Dict]:
        ...
        """
        try:
            # Move string preparation out of query for speed
            lookup_name = name.replace(' ', '').lower()
            db = transaction if transaction else self.db

            # Use AQL's TOLOWER and REMOVE_REGEX for spaces, which is comparable to original
            query = """
            FOR app IN @@collection
                LET normalized = TOLOWER(REGEX_REPLACE(app.name, '\\s+', ''))
                FILTER normalized == @lookup_name
                RETURN app
            """
            cursor = db.aql.execute(query, bind_vars={
            ...
            # Avoid unnecessary cursor iteration/allocation if just one result is needed
            result = next(cursor, None)
            if result:
