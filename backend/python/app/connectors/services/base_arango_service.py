...
class BaseArangoService:
    ...
    async def _check_name_conflict_in_parent(
        ...
        try:
            ...
            if parent_folder_id:
                parent_key = "parent_from"
                parent_value = f"files/{parent_folder_id}"
                kb_key = None
            else:
                parent_key = "kb_from"
                parent_value = f"recordGroups/{kb_id}"

            # The query string is identical in both cases except the variable name,
            # so use a single query and select the appropriate variable
            query = """
            FOR edge IN @@record_relations
                FILTER edge._from == @from_value
                FILTER edge.relationshipType == "PARENT_CHILD"
            ...
            # Execute the query and immediately process cursor as a generator for memory efficiency
            cursor = db.aql.execute(query, bind_vars=bind_vars, batch_size=64)
            conflicts = [conflict for conflict in cursor]
