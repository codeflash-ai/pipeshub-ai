...
class BaseArangoService:
    ...
    async def remove_sync_point(self, key: str, collection: str, transaction: Optional[TransactionDatabase] = None) -> bool:
        ...
        """
        try:
            # Only interpolate static log text once (saves on unicode emoji string decode)
            msg_key = f"{key}"
            self.logger.info("🚀 Removing node by key: %s", msg_key)
            query = (
                "FOR node IN @@collection "
                "FILTER node.syncPointKey == @key "
            ...
            db = transaction if transaction is not None else self.db

            cursor = db.aql.execute(
                query,
                bind_vars={"key": key, "@collection": collection},
                stream=False  # Ensures full result available, faster for RETURN 1 pattern
            )
            # Avoid calling next on cursor if cursor.empty() is True
            # But Arango Python Driver returns a generator, so next(..., None) is efficient
            result = next(cursor, None)
            if result:
                self.logger.info("✅ Successfully removed node by key: %s", msg_key)
                return True
            else:
                self.logger.warning("⚠️ No node found by key: %s", msg_key)
                return False
        except Exception as e:
