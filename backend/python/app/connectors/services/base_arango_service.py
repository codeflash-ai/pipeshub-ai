...
class BaseArangoService:
    ...
    async def get_entity_id_by_email(
        ...
        try:
            ...
            # Use the UNION feature in Arango's AQL to perform both lookups in a single query for better performance.
            # This avoids two roundtrips and leverages the DB's set operations for more efficiency.
            query = """
            FOR doc IN UNION([
                (FOR u IN users FILTER u.email == @email RETURN u._key),
                (FOR g IN groups FILTER g.email == @email RETURN g._key)
            ], true) LIMIT 1 RETURN doc
            """
            result = db.aql.execute(query, bind_vars={"email": email})
            entity_id = next(result, None)

            if entity_id:
                # We don't know if user or group, so message generically
                self.logger.info("✅ Got Entity ID: %s", entity_id)
                return entity_id
