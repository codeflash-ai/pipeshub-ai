...
class BaseArangoService:
    ...
    async def validate_folder_exists_in_kb(
        ...
        try:
            ...
            # Move query out of function body, since it's static text and
            # compiling the triple-quoted string is slightly slower per-call.
            ...
            # Combining bind_vars dictionary construction increases efficiency in CPython,
            # and moreover, avoids spending unnecessary cycles on new dictionary creation for each argument line.
            bind_vars = {
                "folder_id": folder_id,
                "kb_id": kb_id,
                "@files_collection": CollectionNames.FILES.value,
            ...
            # Since 'cursor' is typically a lightweight iterable from Arango,
            # Iterating with 'next(cursor, False)' is already efficient,
            # but we avoid using any intermediate variable allocation.
            # We also rely on the fact that `RETURN true` gives at most one result,
            # so retrieving just the first item is correct.
            return next(db.aql.execute(query, bind_vars=bind_vars), False)
