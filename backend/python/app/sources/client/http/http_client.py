...
class HTTPClient(IClient):
    ...
    async def execute(self, request: HTTPRequest, **kwargs) -> HTTPResponse:
        ...
        # Optimize: Only format the URL if necessary
        url = request.url
        # If there are still any formatting placeholders, apply path param formatting
        if '{' in url and '}' in url and request.path_params:
