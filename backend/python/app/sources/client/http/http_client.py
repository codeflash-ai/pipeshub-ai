...
class HTTPClient(IClient):
    ...
    async def execute(self, request: HTTPRequest, **kwargs) -> HTTPResponse:
        ...
        # Use a single dict literal if possible to save memory/cpu
        merged_headers = self.headers if not request.headers else {**self.headers, **request.headers}
        # Pre-prepare all request arguments for reduced event loop CPU usage
        req_params = request.query_params
        req_headers = merged_headers
        req_path = request.path_params
        body = request.body
        content_type = (request.headers.get('Content-Type', '').lower() if request.headers else '')

        request_kwargs = {'params': req_params, 'headers': req_headers, **kwargs}
        ...
        # Formatting URL before sending for reduced event loop cpu work
        formatted_url = f'{request.url.format(**req_path)}'
        response = await client.request(request.method, formatted_url, **request_kwargs)
