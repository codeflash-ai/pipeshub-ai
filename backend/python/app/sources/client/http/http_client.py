...
class HTTPClient(IClient):
    ...
    async def execute(self, request: HTTPRequest, **kwargs) -> HTTPResponse:
        ...
        url_fmt = f'{request.url.format(**request.path_params)}' if request.path_params else request.url
        response = await client.request(request.method, url_fmt, **request_kwargs)
