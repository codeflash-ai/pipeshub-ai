...
class HTTPClient(IClient):
    ...
    async def execute(self, request: HTTPRequest, **kwargs) -> HTTPResponse:
        ...
        # Merge client headers with request headers, prefer request headers (dict unpacking order)
        if request.headers:
            merged_headers = {**self.headers, **request.headers}
        else:
            merged_headers = self.headers

        # Only include params if not empty (avoids unnecessary dict allocations)
        request_kwargs = {}
