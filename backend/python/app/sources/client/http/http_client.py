...
class HTTPClient(IClient):
    ...
    async def execute(self, request: HTTPRequest, **kwargs) -> HTTPResponse:
        ...
        """
        # Avoid redundant format if path_params is empty or only contains string keys
        path_params = request.path_params
        if path_params:
            url = f"{request.url.format(**path_params)}"
        else:
            url = request.url

        client = await self._ensure_client()

        # Faster header merging (request headers take precedence)
        if request.headers:
