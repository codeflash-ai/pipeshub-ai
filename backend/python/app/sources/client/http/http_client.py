...
class HTTPClient(IClient):
    ...
    async def execute(self, request: HTTPRequest, **kwargs) -> HTTPResponse:
        ...
        request_kwargs = {'params': request.query_params if request.query_params else None, 'headers': merged_headers, **kwargs}
        body = request.body
        content_type = ''
        if request.headers:
        ...
        # Avoid path param formatting if empty
        path_params = request.path_params if request.path_params else {}
        url = request.url.format(**path_params) if path_params else request.url
