...
class HTTPClient(IClient):
    ...
    async def execute(self, request: HTTPRequest, **kwargs) -> HTTPResponse:
        ...
        request_kwargs = dict(params=request.query_params, headers=merged_headers)
        ...
        response = await client.request(request.method, request.url.format(**request.path_params), **request_kwargs)
