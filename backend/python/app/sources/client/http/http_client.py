...
class HTTPClient(IClient):
    ...
    async def execute(self, request: HTTPRequest, **kwargs) -> HTTPResponse:
        ...
        # Reduce dict merge overhead by using dict unpacking only when necessary.
        request_headers = request.headers
        merged_headers = {**self.headers, **request_headers} if request_headers else self.headers
        request_kwargs = {'params': request.query_params, 'headers': merged_headers, **kwargs}
        body = request.body
        request_content_type = request_headers.get('Content-Type', '').lower() if request_headers else ''
        ...
        # Path parameter formatting is done eagerly for url.
