...
class HTTPClient(IClient):
    ...
    async def execute(self, request: HTTPRequest, **kwargs) -> HTTPResponse:
        ...
        if isinstance(body, dict):
            ...
            content_type = (
                request.headers.get("Content-Type", "") if request.headers else ""
            ).lower()
        ...
        # Remove None values that may be present in request_kwargs
        filtered_request_kwargs = {
            k: v for k, v in request_kwargs.items() if v is not None
        }

        response = await client.request(request.method, url, **filtered_request_kwargs)
