...
class HTTPClient(IClient):
    ...
    async def execute(self, request: HTTPRequest, **kwargs) -> HTTPResponse:
        ...
        # Only allocate a new merged_headers dict if request_headers is present and non-empty
        if request_headers:
        ...
        body = request.body
        if body is not None:
            content_type = request_headers.get('Content-Type', '').lower() if request_headers else ''
        ...
        # Only format the URL if there are path params
        url = request.url
