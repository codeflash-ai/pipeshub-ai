...
class HTTPClient(IClient):
    ...
    async def execute(self, request: HTTPRequest, **kwargs) -> HTTPResponse:
        ...
        # Branch only once for merged headers
        merged_headers = self.headers.copy()
        if request.headers:
            merged_headers.update(request.headers)
        # Move path_params defaulting to inside url format to avoid unnecessary copying
        request_kwargs = {
        ...
        # Avoid dict copy for path_params -- use directly
