...
class HTTPClient(IClient):
    ...
    async def execute(self, request: HTTPRequest, **kwargs) -> HTTPResponse:
        ...
        request_kwargs = {'params': request.query_params or {}, 'headers': merged_headers, **kwargs}
        body = request.body
        ...
        url_formatted = request.url.format(**(request.path_params or {}))
        response = await client.request(request.method, url_formatted, **request_kwargs)
