...
class HTTPClient(IClient):
    ...
    async def execute(self, request: HTTPRequest, **kwargs) -> HTTPResponse:
        ...
        # Avoid unnecessary copy if request.headers is falsy
        headers = self.headers.copy()
        if request.headers:
            headers.update(request.headers)
        # Move param construction outside branch for clarity
        request_kwargs = {
        ...
        # Consolidate body handling for dict/bytes, only set the right parameter
        ...
        # Precompute the formatted URL (faster and less error-prone than repeated formatting)
        formatted_url = request.url
        if request.path_params:
            try:
                formatted_url = formatted_url.format(**request.path_params)
            except Exception:
                pass
