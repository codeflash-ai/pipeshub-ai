...
class HTTPClient(IClient):
    ...
    async def execute(self, request: HTTPRequest, **kwargs) -> HTTPResponse:
        ...
        # Fast-path: empty path_params means avoid format call
        if not request.path_params:
            url_to_use = request.url
        else:
            url_to_use = f'{request.url.format(**request.path_params)}'

        response = await client.request(request.method, url_to_use, **request_kwargs)
