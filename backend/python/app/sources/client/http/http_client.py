...
class HTTPClient(IClient):
    ...
    async def execute(self, request: HTTPRequest, **kwargs) -> HTTPResponse:
        ...
        # Only copy self.headers if combining, else use as-is for perf
        if req_headers:
        ...
        body = request.body
        content_type = req_headers.get('Content-Type', '').lower() if req_headers else ''
        ...
        # Only format path params if present for efficiency
        if request.path_params:
            req_url = request.url.format(**request.path_params)
        else:
            req_url = request.url
        response = await client.request(request.method, req_url, **request_kwargs)
