...
class HTTPClient(IClient):
    ...
    async def execute(self, request: HTTPRequest, **kwargs) -> HTTPResponse:
        ...
        # Merge headers (request headers override client headers) efficiently
        if request.headers:
        ...
        body = request.body
        # Combine type checks for body; assign with minimal branching
        content_type = (
            request.headers.get("Content-Type", "").lower() if request.headers else ""
        )
