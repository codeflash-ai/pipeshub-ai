...
class HTTPClient(IClient):
    ...
    async def execute(self, request: HTTPRequest, **kwargs) -> HTTPResponse:
        ...
        if request.headers:
            # direct update is faster than .copy+update for small dicts, but must not mutate self.headers
            req_headers = {**self.headers, **request.headers}
        request_kwargs = {'params': request.query_params, 'headers': req_headers, **kwargs}
        body = request.body
