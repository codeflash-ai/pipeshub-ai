...
class HTTPClient(IClient):
    ...
    async def execute(self, request: HTTPRequest, **kwargs) -> HTTPResponse:
        ...
        # Optimization: inline path param formatting only if needed and use local references
        request_url = request.url
        path_params = request.path_params
        if path_params:
            request_url = request_url.format(**path_params)
        response = await client.request(request.method, request_url, **request_kwargs)
