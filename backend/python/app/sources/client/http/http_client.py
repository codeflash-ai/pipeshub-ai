...
class HTTPClient(IClient):
    ...
    async def execute(self, request: HTTPRequest, **kwargs) -> HTTPResponse:
        ...
        request_kwargs = {'params': request.query_params, 'headers': merged_headers}
        ...
        url_rendered = f"{request.url.format(**request.path_params)}"
        response = await client.request(request.method, url_rendered, **request_kwargs)
