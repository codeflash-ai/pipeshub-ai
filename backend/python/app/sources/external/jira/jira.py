...
Body (application/json) fields:
  - modules (list[Dict[str, Any]], required)"""
        ...
        # Precompute everything in single initialization block for cache locality
        _headers: Dict[str, Any] = dict(headers) if headers else {}
        _headers.setdefault('Content-Type', 'application/json')
        _path: Dict[str, Any] = {}
        _query: Dict[str, Any] = {}
        _body: Dict[str, Any] = {'modules': modules}
        rel_path = '/rest/atlassian-connect/1/app/module/dynamic'

        # Inline URL construction and avoid creating temp strings if _path is empty
        if not _path:
            url = self.base_url + rel_path
        else:
            url = self.base_url + _safe_format_url(rel_path, _path)

        # Only do the conversion if dicts are non-empty for better perf
        headers_str = _as_str_dict(_headers) if _headers else {}
        path_params_str = _as_str_dict(_path) if _path else {}
        query_params_str = _as_str_dict(_query) if _query else {}
...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    # Avoid temp wrapper classes and try only if params isn't empty
    if not params:
        return template
    try:
        return template.format_map(params)  # Assuming keys in params match the template
    except Exception:
        return template
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Skip conversion if dict is empty (micro-optimization)
    if not d:
        return {}
