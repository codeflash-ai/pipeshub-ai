...
Body (application/json) fields:
  ...
  - workflow (str, optional)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Combine headers efficiently
        if headers:
            _headers: Dict[str, Any] = {**headers}
        else:
            _headers: Dict[str, Any] = {}
        _headers.setdefault('Content-Type', 'application/json')
        _path = {'id': id, 'issueType': issueType}
        _query = {}
        # Only add fields to _body if present for faster allocation and less memory
        _body = {}
        if issueType_body is not None:
            _body['issueType'] = issueType_body
        if updateDraftIfNeeded is not None:
        ...
        rel_path = '/rest/api/3/workflowscheme/{id}/issuetype/{issueType}'
        url = self.base_url + _safe_format_url(rel_path, _path)
        # The _as_str_dict functions are not async but can be replaced with dict comprehensions for clarity and performance
        req = HTTPRequest(
            method='PUT',
            url=url,
            headers={str(k): _serialize_value(v) for (k, v) in _headers.items()},
            path_params={str(k): _serialize_value(v) for (k, v) in _path.items()},
            query_params={},  # _query is always empty here
            body=_body
        )
        resp = await self._client.execute(req)
        return resp
