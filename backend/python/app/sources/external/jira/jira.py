...
Path params:
  - issueLinkTypeId (str)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Avoid creating new dicts more than necessary
        if headers:
            _headers = dict(headers)
        else:
            _headers = {}
        # Path and query parameters are static structure; serialize once, reuse
        _path = {'issueLinkTypeId': issueLinkTypeId}
        _query = {}
        _body = None
        rel_path = f'/rest/api/3/issueLinkType/{issueLinkTypeId}'
        # Use direct f-string for url, reduces format_map overhead and SafeDict construction
        url = self.base_url + rel_path
        _headers_str = _as_str_dict(_headers)
        _path_str = {'issueLinkTypeId': _serialize_value(issueLinkTypeId)}
        # _query is always {}, avoid unnecessary function call
        req = HTTPRequest(
            method='DELETE',
            url=url,
            headers=_headers_str,
            path_params=_path_str,
            query_params={},  # It's always empty, do not serialize
            body=_body
        )
        resp = await self._client.execute(req)
        return resp
