...
Body (application/json) fields:
  ...
  - validationOptions (Dict[str, Any], optional)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Merge headers in a single, faster step
        default_headers = {'Content-Type': 'application/json'}
        _headers: Dict[str, Any] = {**default_headers, **(headers or {})}
        _path: Dict[str, Any] = {}
        _query: Dict[str, Any] = {}
        # This body allocation is minimal and lean
        _body: Dict[str, Any] = {'payload': payload}
        if validationOptions is not None:
            _body['validationOptions'] = validationOptions
