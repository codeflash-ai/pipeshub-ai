...
Query params:
  ...
  - expand (str, optional)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        _headers: Dict[str, Any] = headers if headers is not None else {}
        # Batch construction of query parameters reduces Python frame/bytecode overhead
        _params = (
            ('startAt', startAt),
        ...
        _query: Dict[str, Any] = {k: v for k, v in _params if v is not None}
        _path: Dict[str, Any] = {}
        _body = None
        rel_path = '/rest/api/3/priorityscheme'
        # Fast-path when _path is empty: skip .format_map
        url = (
...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    # Fast-path if params are empty: no formatting needed
    if not params:
        return template
    try:
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # For empty dict or single-key, minimize iteration overhead
    if not d:
        return {}
    if len(d) == 1:
