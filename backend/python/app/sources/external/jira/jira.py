...
# Constants to avoid unnecessary allocations for empty cases.
...
Path params:
  ...
  - propertyKey (str)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Use module-level constants to avoid empty dict allocations unless needed
        ...
        # Empty query dict is always the same
        _query: Dict[str, Any] = _EMPTY_DICT
        _body = None
        rel_path = '/rest/api/3/project/{projectIdOrKey}/properties/{propertyKey}'
        # Convert input dicts to string dicts *once* and re-use for all needed parts
        str_headers = _as_str_dict(_headers) if _headers else _EMPTY_STR_DICT
        str_path = _as_str_dict(_path)
        # Since _query is always empty, just keep _EMPTY_STR_DICT for _query
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # This mirrors the referenced serialization, does not inline for clarity and reuse
    return {str(k): _serialize_value(v) for (k, v) in d.items()}

def _to_bool_str(x: Any) -> str:
    # Matches behavior in referenced read-only _serialize_value's helpers
