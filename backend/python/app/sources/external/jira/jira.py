...
Body (application/json) fields:
  - locale (str, optional)"""
        ...
        # These dicts are always empty, so avoid repeated allocation by using the module-level constant
        _path = _EMPTY_DICT
        _query = _EMPTY_DICT

        _body = {'locale': locale} if locale is not None else _EMPTY_DICT

        rel_path = '/rest/api/3/mypreferences/locale'

        # Fast-path: if _path is always empty, don't call _safe_format_url
        url = self.base_url + rel_path

        # Only call _as_str_dict if headers is not empty (same for path/query)
        as_str_dict = _as_str_dict

        req = HTTPRequest(
            method='PUT',
            url=url,
            headers=as_str_dict(_headers) if _headers else _EMPTY_DICT,
            path_params=_EMPTY_DICT,  # always {}
            query_params=_EMPTY_DICT,  # always {}
            body=_body
        )
        resp = await self._client.execute(req)
        return resp
