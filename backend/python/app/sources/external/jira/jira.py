...
Body (application/json) fields:
  ...
  - position (str, optional)"""
        ...
        # Pre-allocate headers only once; avoid dict copying if not needed
        if headers:
            _headers: Dict[str, Any] = dict(headers)
            _headers['Content-Type'] = _headers.get('Content-Type', 'application/json')
        else:
        ...
        # Inline fastpath _as_str_dict for single-entry dicts
        req = HTTPRequest(
            method='PUT',
            url=url,
            headers={str(k): _serialize_value(v) for (k, v) in _headers.items()} if _headers else {},
            path_params={"issueTypeSchemeId": str(issueTypeSchemeId)},
            query_params={},
            body=_body
        )
        resp = await self._client.execute(req)
...
def _fast_safe_format_url(template: str, params: Dict[str, object]) -> str:
    # Fast path for single {issueTypeSchemeId}
    # Only fallback to full .format_map if needed
    try:
        # If exactly the expected key, fast replace
        if "{issueTypeSchemeId}" in template:
            v = params.get("issueTypeSchemeId")
            if v is not None:
                return template.replace("{issueTypeSchemeId}", str(v))
        return template.format_map(_SafeDict(params))
    except Exception:
        return template
