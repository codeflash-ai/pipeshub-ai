...
# Pre-allocated constants for empty param reuse
_EMPTY_DICT_STR: Dict[str, str] = {}
_EMPTY_DICT_ANY: Dict[str, Any] = {}
...
Query params:
  - key (str, optional)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Don't wrap in dict unless needed
        if headers:
            _headers: Dict[str, Any] = dict(headers)
        else:
            _headers = _EMPTY_DICT_ANY
        _path = _EMPTY_DICT_ANY
        # Only allocate and insert if key is provided!
        if key is not None:
            _query = {'key': key}
        else:
            _query = _EMPTY_DICT_ANY
        _body = _BODY_NONE
        rel_path = '/rest/api/3/projectvalidate/key'
        base_url = self.base_url  # Cache lookup
        # Fast path for rel_path: always no '{' in template (no format needed)
        url = base_url + rel_path
        req = HTTPRequest(
            method='GET', 
            url=url, 
            headers=_as_str_dict(_headers), 
            path_params=_EMPTY_DICT_STR, 
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Fast path for empty dict (major time saver per profiling)
    if not d:
        return _EMPTY_DICT_STR
