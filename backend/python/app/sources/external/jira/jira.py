...
Query params:
  - componentSource (str, optional)"""
        ...
        if len(_path) == 1 and 'projectIdOrKey' in _path:
            # Fast path for single-placeholder template, avoids .format_map and SafeDict alloc
            url = f"{self.base_url}/rest/api/3/project/{_path['projectIdOrKey']}/components"
        else:
            url = self.base_url + _safe_format_url(rel_path, _path)
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # List comprehension is marginally faster for tiny dicts; preserves full conversion logic
