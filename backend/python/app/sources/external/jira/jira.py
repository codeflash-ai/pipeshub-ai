...
_REL_PATH = '/rest/api/3/field/{fieldId}/context/defaultValue'
...
Body (application/json) fields:
  - defaultValues (list[Dict[str, Any]], optional)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Avoid unnecessary allocations/computations on trivial/empty dicts.
        ...
        # Inline rel_path string usage (now as class-level constant)
        url = f"{self.base_url}{_safe_format_url(_REL_PATH, _path)}"

        # Only apply serialization if dict is non-empty and/or contains non-string values
        req = HTTPRequest(
            method='PUT',
            url=url,
            headers=_as_str_dict(_headers) if _headers else _EMPTY_STR_DICT,
            path_params={'fieldId': fieldId},  # Fast path: guaranteed str
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Optimize for common patterns and trivially-stringable keys/values
    if not d:
        return _EMPTY_STR_DICT
    # If all values are already strings, cheaper than _serialize_value
