...
Body (application/json) fields:
  ...
  - additionalProperties allowed (pass via body_additional)"""
        ...
        # Use a faster pattern for optional dict construction and fewer dict creations
        # Set Content-Type if not present
        if headers is None:
            _headers = {'Content-Type': 'application/json'}
        else:
            # Only setdefault if it is not present, don't copy if not needed
        ...
        # Initialize path dict once
        _path = {'issueIdOrKey': issueIdOrKey, 'id': id}

        # More efficient query dict (single dict, inline short-circuit assignment)
        # Build only keys that have values (no .setdefault overhead)
        _query = {}
        if notifyUsers is not None:
            _query['notifyUsers'] = notifyUsers
        if overrideEditableFlag is not None:
        ...
        # Compose _body efficiently by collecting present (not-None) keys
        _body = {}
        # These are all optional fields: branch minimizes attribute lookups and dictionary updates
        ...
        # Avoid redundant repeat calls to _as_str_dict for same inputs (no changes within _headers, _path, _query)
        _headers_str = _as_str_dict(_headers) if _headers else {}
        _path_str = _as_str_dict(_path)
        _query_str = _as_str_dict(_query) if _query else {}

        req = HTTPRequest(
            method='PUT',
            url=url,
            headers=_headers_str,
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Inline generator and direct-to-dict is fastest
