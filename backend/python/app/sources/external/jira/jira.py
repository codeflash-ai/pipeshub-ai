...
Body (application/json) fields:
  ...
  - expressions (list[str], required)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Avoid unnecessary dict copy/allocation; short circuit empty
        if headers:
        ...
        # Pre-allocate and reuse empty dict objects
        _path: Dict[str, Any] = {}
        if check is not None:
            _query: Dict[str, Any] = {'check': check}
        else:
            _query: Dict[str, Any] = {}
        if contextVariables is not None:
            _body: Dict[str, Any] = {'contextVariables': contextVariables, 'expressions': expressions}
        else:
            _body: Dict[str, Any] = {'expressions': expressions}

        rel_path = '/rest/api/3/expression/analyse'
        url = self.base_url + _safe_format_url(rel_path, _path)
        # Avoid dict conversion for empty dict, and skip unneeded conversion
        req = HTTPRequest(
            method='POST',
            url=url,
...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    # Fast-path: if params is empty or template has no {, skip formatting
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Fast-path, avoid unnecessary comprehension if empty
    if not d:
        return {}
