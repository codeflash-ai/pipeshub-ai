...
Body (application/json) fields:
  ...
  - additionalProperties allowed (pass via body_additional)"""
        ...
        # Use local dict to batch query param population faster with key-value pairs
        _query: Dict[str, Any] = {
            k: v for k, v in [
        ...
        # Assemble body dict using dict literal for speed
        _body: Dict[str, Any] = {
            **({'author': author} if author is not None else {}),
            **({'comment': comment} if comment is not None else {}),
            **({'created': created} if created is not None else {}),
            **({'id': id_body} if id_body is not None else {}),
            **({'issueId': issueId} if issueId is not None else {}),
            **({'properties': properties} if properties is not None else {}),
            **({'self': self_} if self_ is not None else {}),
            **({'started': started} if started is not None else {}),
            **({'timeSpent': timeSpent} if timeSpent is not None else {}),
            **({'timeSpentSeconds': timeSpentSeconds} if timeSpentSeconds is not None else {}),
            **({'updateAuthor': updateAuthor} if updateAuthor is not None else {}),
            **({'updated': updated} if updated is not None else {}),
            **({'visibility': visibility} if visibility is not None else {})
        }
        # If additional body data is present, update body dict.
        if body_additional:
            _body.update(body_additional)
        rel_path = '/rest/api/3/issue/{issueIdOrKey}/worklog/{id}'
        url = self.base_url + _safe_format_url(rel_path, _path)
        
        # Run dictionary encoding concurrently to take advantage of awaitable client execute immediately after
        # The _as_str_dict helper is synchronous, but minimize awaitable delay before client execution
        req = HTTPRequest(
            method='PUT',
            url=url,
        ...
        # Await client execution directly instead of intermediate variable assignment
