...
Body (application/json) fields:
  ...
  - name (str, required)"""
        ...
        req = HTTPRequest(
            ...
            headers=_as_str_dict(_headers),
            path_params=_as_str_dict(_path),
            query_params={},   # avoid unnecessary calls/copies, static empty
            body=_body,
        )
        resp = await self._client.execute(req)
        return resp
...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    # Optimized for single '{id}' replacement, common Jira REST pattern
    if '{id}' in template and len(params) == 1 and 'id' in params:
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    ...
    # Fast-path for empty dicts (common in query/path_params)
    if not d:
        return {}
