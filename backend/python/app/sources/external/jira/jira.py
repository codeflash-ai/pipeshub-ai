...
# Module-level constant for empty dict to avoid repeated construction/allocation
...
Path params:
  ...
  - id (int)"""
        ...
        # Use fast path for empty/missing headers
        if headers:
            _headers: Dict[str, Any] = dict(headers)
            req_headers = _as_str_dict(_headers)
        else:
            # Avoid the dict-allocation and comprehension if not needed
            req_headers = {}

        # _path is always the same construction
        _path: Dict[str, Any] = {'type': type, 'owningObjectId': owningObjectId, 'id': id}
        req_path_params = _as_str_dict(_path)

        # _query always empty - use module-level constant and avoid function call
        req_query_params = _EMPTY_DICT
...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    # Fast-path: if template has no '{', skip formatting
    if '{' not in template:
        return template
    try:
