...
Path params:
  - id (int)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Directly use headers or empty dict without extra copy
        _headers = headers if headers is not None else {}
        # _path is always {'id': id}; avoid unnecessary dict ops in tight loop
        _path = {'id': id}
        # _query is always empty -- avoid dict conversion, pass as empty dict
        _query = {}
        _body = None
        rel_path = '/rest/api/3/workflowscheme/{id}/createdraft'
        # Inline the URL formatting for this hot-path (only variable is id)
        url = f"{self.base_url}/rest/api/3/workflowscheme/{id}/createdraft"
        # Inline str dict for _path; _as_str_dict(_query) -> {} always
        req = HTTPRequest(
            method='POST',
            url=url,
