...
Path params:
  - id (str)"""
        ...
        # Optimize dict usage: avoid allocation for empty cases
        _headers: Dict[str, Any] = dict(headers) if headers else {}
        # Optimize: inline path dict for common case
        rel_path = '/rest/api/3/field/{id}/restore'
        url = self.base_url + rel_path.format(id=id)
        # Pre-convert dicts only if needed (these are often trivial)
        req = HTTPRequest(
            method='POST',
            url=url,
            headers=_as_str_dict(_headers) if _headers else {},
            path_params={"id": str(id)},
            query_params={},  # always empty; no need to convert
            body=None,
        )
        resp = await self._client.execute(req)
        return resp
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    ...
    # Return immediately if dict is empty (fast-path for frequent empty input)
    if not d:
        return {}
