...
# Inline SafeDict for efficient format_map fallback handling
...
Path params:
  - schemeId (int)"""
        ...
        # Direct assignment and avoid unnecessary empty dict instances
        headers_ = headers if headers is not None else {}
        path_params = {'schemeId': schemeId}

        # Pre-convert str-dict once per type (avoid multiple comprehensions below)
        as_str_headers = _as_str_dict(headers_) if headers_ else {}
        as_str_path = {'schemeId': str(schemeId)}  # Shortcut, only one known int param
        as_str_query = {}  # Known to be empty always

        # Use list join for URL to avoid string concat in the hot path
        rel_path = '/rest/api/3/priorityscheme/{schemeId}'
        url = f"{self.base_url}{_safe_format_url(rel_path, path_params)}"
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Move import inside for performance (if not always needed)
    from app.sources.external.jira.jira import _serialize_value
    return {str(k): _serialize_value(v) for (k, v) in d.items() if v is not None}
