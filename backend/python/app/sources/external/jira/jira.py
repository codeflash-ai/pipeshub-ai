...
Body (application/json) fields:
  ...
  - sendBulkNotification (bool, optional)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Use a single dict only if 'headers' is set to avoid unnecessary copying
        if headers:
            _headers: Dict[str, Any] = dict(headers)
            _headers.setdefault('Content-Type', 'application/json')
        else:
            _headers: Dict[str, Any] = {'Content-Type': 'application/json'}
        # Avoid unnecessary dict creation if path/query is always empty
        _path: Dict[str, Any] = {}
        _query: Dict[str, Any] = {}
        _body: Dict[str, Any] = {}
        ...
        if sendBulkNotification is not None:
            _body['sendBulkNotification'] = sendBulkNotification
        rel_path = '/rest/api/3/bulk/issues/fields'
        # Optimize URL formation if _path is empty
        url = self.base_url + (rel_path if not _path else _safe_format_url(rel_path, _path))
        # Avoid comprehensions for empty dicts
        req = HTTPRequest(
            method='POST',
            url=url,
            headers=_as_str_dict(_headers),
            path_params=_path if not _path else _as_str_dict(_path),
            query_params=_query if not _query else _as_str_dict(_query),
            body=_body
        )
        resp = await self._client.execute(req)
        return resp
