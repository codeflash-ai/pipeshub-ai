...
Body (application/json) fields:
  ...
  - jqls (list[str], required)"""
        ...
        content_type_value = _headers.setdefault('Content-Type', 'application/json')
        rel_path = '/rest/api/3/jql/match'
        url = self.base_url + rel_path  # Optimization: _path is always empty
        # Only convert headers to str if needed (user-passed headers)
        headers_as_str = _as_str_dict(_headers) if _headers else {}
        # _path and _query are always empty, so pass {} directly
        req = HTTPRequest(
            method='POST',
            url=url,
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Micro-opt: skip str() for string keys, call _serialize_value as needed
