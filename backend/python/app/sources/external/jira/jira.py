...
Path params:
  ...
  - contextId (int)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        _headers: Dict[str, Any] = dict(headers or {})
        # Avoid _as_str_dict conversion on static paths and queries for extra speed
        # Since fieldId and contextId are guaranteed present and correctly typed
        # inline str conversion below
        url = f"{self.base_url}/rest/api/3/field/{fieldId}/context/{contextId}"
        req = HTTPRequest(
            method='DELETE',
            url=url,
            headers=_as_str_dict(_headers),
            path_params={"fieldId": str(fieldId), "contextId": str(contextId)},  # minimal conversion since values are known
            query_params={},  # always empty, avoid conversion
            body=None
        )
        resp = await self._client.execute(req)
        return resp
