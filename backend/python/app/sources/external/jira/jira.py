...
Path params:
  - id (str)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Inline: Faster than dict(headers or {}) for the common (None/empty) case
        _headers: Dict[str, Any] = headers.copy() if headers else {}
        # Avoid repeated function calls by directly formatting the URL
        url = f"{self.base_url}/rest/api/3/resolution/{_serialize_value(id)}"
        # Inline trivial dicts for efficiency
        req = HTTPRequest(
            method='GET',
            url=url,
            headers=_as_str_dict(_headers),
            path_params={'id': _serialize_value(id)},  # Already a string due to _serialize_value
            query_params={},
            body=None
        )
        resp = await self._client.execute(req)
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Optimize single-key and small dicts for performance
    if not d:
        return {}
    if len(d) == 1:
