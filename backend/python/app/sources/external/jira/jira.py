...
Path params:
  ...
  - propertyKey (str)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Fast path for headers: avoid redundant dict construction if not needed
        _headers = headers if headers is not None else {}
        # Path params always are str; skip full stringified conversion
        _path = {'projectIdOrKey': projectIdOrKey, 'propertyKey': propertyKey}
        # Query params always empty; avoid function call
        _query = {}
        _body = None
        rel_path = '/rest/api/3/project/{projectIdOrKey}/properties/{propertyKey}'
        base_url = self.base_url     # Single attribute lookup

        url = base_url + _safe_format_url(rel_path, _path)
        req = HTTPRequest(
            method='GET',
            url=url,
            headers=_as_str_dict(_headers),   # Only call if headers dict is not empty
            path_params=_path,                # Path keys/values are already str
            query_params=_query,              # Always empty dict
            body=_body
        )
        resp = await self._client.execute(req)
        return resp
...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    """Safely format URL template. If formatting fails, return template unchanged."""
    # Only call format_map if all keys required are in params
    try:
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    """Convert all keys and values to string, serializing values as needed."""
    # Short-circuit for empty dicts
    if not d:
        return {}
    # If all values are strings, return fast
