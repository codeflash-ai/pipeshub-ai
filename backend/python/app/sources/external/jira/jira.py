...
Path params:
  - projectKeyOrId (str)"""
        ...
        url = self.base_url + _safe_format_url(rel_path, {'projectKeyOrId': projectKeyOrId})
        req = HTTPRequest(
            method='GET',
            url=url,
            headers=_as_str_dict(headers or {}),
            path_params={'projectKeyOrId': str(projectKeyOrId)},
            query_params={},
            body=None
        )
        resp = await self._client.execute(req)
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # dict comprehension using generator for better memory in async contexts
