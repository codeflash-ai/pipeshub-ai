...
HTTP GET /rest/api/3/attachment/meta"""
        ...
        # Optimize path formatting: fast path if _path is empty
        url = self.base_url + (_safe_format_url(rel_path, _path) if _path else rel_path)
        # Optimize _as_str_dict: fast path for empty dicts
        req = HTTPRequest(
            method='GET',
            url=url,
...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    # Fast path: avoid format_map machinery if params is empty
    if not params:
        return template
    try:
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Fast path: avoid dict comp/allocation if already empty
    if not d:
        return {}
