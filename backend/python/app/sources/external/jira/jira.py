...
Path params:
  - planId (int)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Avoid unnecessary copying if headers is None
        _headers = headers if headers else {}
        # Directly use planId as string if possible (fast path)
        _path = {'planId': planId}
        _query = {}
        _body = None
        rel_path = '/rest/api/3/plans/plan/{planId}/trash'
        # Only 1 path param, avoid SafeDict
        url = f"{self.base_url}/rest/api/3/plans/plan/{planId}/trash"

        # Optimize conversion for hot path
        req = HTTPRequest(
            method='PUT',
            url=url,
            headers=_as_str_dict(_headers),
            path_params={'planId': str(planId)},
            query_params={},
            body=_body
        )
        resp = await self._client.execute(req)
