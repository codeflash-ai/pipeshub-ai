...
Body (application/json) fields:
  ...
  - additionalProperties allowed (pass via body_additional)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Avoid unnecessary dict constructors
        if headers:
        ...
        # Use inline empty dict assignment, since path/query aren't mutated
        _path: Dict[str, Any] = {}
        _query: Dict[str, Any] = {}
        ...
        rel_path = '/rest/api/3/resolution'
        url = self.base_url + _safe_format_url(rel_path, _path)
        # Bind conversion functions locally for dict applications
        str_fn = str
...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    # Fast-path for the frequent case: params are empty (no subst)
    if not params:
        return template
    try:
