...
# Constant for empty string dict to avoid redundant allocations
...
Path params:
  ...
  - id (int)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Quick header copy: only allocates dict if headers is not None
        _headers: Dict[str, Any] = headers if headers is not None else _EMPTY_STR_DICT
        # Directly construct path dict and convert to string dict inline
        _path_str_dict = {
            'projectIdOrKey': _serialize_value(projectIdOrKey),
            'id': _serialize_value(id)
        }
        # Query is always empty; reuse constant
        _query_str_dict = _EMPTY_STR_DICT
        _body = None
        rel_path = '/rest/api/3/project/{projectIdOrKey}/avatar/{id}'
        url = self.base_url + _safe_format_url(rel_path, {'projectIdOrKey': projectIdOrKey, 'id': id})
        req = HTTPRequest(
            method='DELETE',
            url=url,
            headers=_as_str_dict(_headers),
            path_params=_path_str_dict,
            query_params=_query_str_dict,
            body=_body
        )
        resp = await self._client.execute(req)
        return resp
