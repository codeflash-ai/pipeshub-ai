...
Body (application/json) fields:
  - jql (str, optional)"""
        ...
        # Pre-create and reuse empty dicts for improved memory efficiency.
        _empty_dict: Dict[str, Any] = {}
        _path: Dict[str, Any] = _empty_dict
        _query: Dict[str, Any] = _empty_dict

        # Only create _body if needed.
        if jql is not None:
            _body: Dict[str, Any] = {'jql': jql}
        else:
            _body: Dict[str, Any] = _empty_dict

        rel_path = '/rest/api/3/issue/archive'
        # Optimized format: _path is always empty dict; we can avoid formatting.
        url = self.base_url + rel_path

        # Use optimized helper for dicts.
        req = HTTPRequest(
            method='POST',
            url=url,
            headers=_as_str_dict(_headers),
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    ...
    if not d:
        return {}
    # Use a list comprehension for slightly faster conversion
