...
# Pre-convert constant empty dicts to string dicts for reuse
...
Path params:
  - id (str)"""
        ...
        # Avoid redundant dict conversions and allocations
        _headers = headers if headers is not None else {}
        rel_path = '/rest/api/3/priority/{id}'
        # Fast inline conversion for tiny dicts
        path_param = {'id': id}
        url = f"{self.base_url}{_safe_format_url(rel_path, path_param)}"

        # Precompute string dicts for small/static values
        str_headers = _as_str_dict(_headers) if _headers else _EMPTY_STR_DICT
        str_path_param = {'id': _serialize_value(id)}
        # _query and _body are always empty/None for this endpoint
        req = HTTPRequest(
            method='GET',
            url=url,
            headers=str_headers,
            path_params=str_path_param,
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Tiny dicts, avoid comprehension when empty
    if not d:
