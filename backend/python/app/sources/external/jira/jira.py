...
Path params:
  ...
  - permissionId (int)"""
        ...
        _path: Dict[str, Any] = {'schemeId': str(schemeId), 'permissionId': str(permissionId)}
        _query: Dict[str, Any] = {}
        _body = None
        rel_path = '/rest/api/3/permissionscheme/{schemeId}/permission/{permissionId}'
        url = self.base_url + _safe_format_url(rel_path, _path)
        req = HTTPRequest(method='DELETE', url=url, headers=_headers, path_params=_path, query_params=_query, body=_body)
        resp = await self._client.execute(req)
        return resp
