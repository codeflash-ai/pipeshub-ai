...
Body: application/json (str)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Inline expansion to avoid repeated function call/alloc
        if headers:
        ...
        _path = {'issueIdOrKey': issueIdOrKey, 'propertyKey': propertyKey}

        # Pre-convert _as_str_dict to avoid repeated calls
        _headers_str = _headers
        _path_str = {str(k): _serialize_value(v) for k, v in _path.items()}

        # _query is always empty, avoid unnecessary conversion
        # Use fast-path for empty
        _query_str = {}
