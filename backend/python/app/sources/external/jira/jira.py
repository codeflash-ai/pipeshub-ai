...
Body (application/json) fields:
  - id (str, required)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Minimize dict creations and redundant conversions
        _body = {'id': id}
        rel_path = '/rest/api/3/resolution/default'
        # _path and _query are always empty, so skip redundant processing
        url = self.base_url + rel_path
        _headers = dict(headers) if headers else {}
        if 'Content-Type' not in _headers:
            _headers['Content-Type'] = 'application/json'
        # Avoid repeated (and expensive) calls to _as_str_dict for trivially empty dicts
