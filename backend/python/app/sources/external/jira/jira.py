...
Body (application/json) fields:
  ...
  - self (str, optional)"""
        rel_path = '/rest/api/3/issueLinkType'
        url = self.base_url + rel_path  # path is always static, no formatting
        
        # Minimize dict construction; avoid checking keys one by one
        ...
        # Headers: only add if needed, minimize dict copying by building direct
        if headers:
        ...
        # Path/query params are always empty for this endpoint
        req = HTTPRequest(
            method='POST',
            url=url,
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Optimize: only convert non-str keys and non-str values
    # In practice, keys for headers and path/query are always str in this codebase
    # If dict is empty, return empty dict immediately
    if not d:
        return {}
    result = {}
    for k, v in d.items():
