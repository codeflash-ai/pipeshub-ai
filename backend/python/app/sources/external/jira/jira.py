...
Body (application/json) fields:
  ...
  - additionalProperties allowed (pass via body_additional)"""
        ...
        # Prepare headers with less copying
        _headers = dict(headers) if headers else {}
        if 'Content-Type' not in _headers:
            _headers['Content-Type'] = 'application/json'

        # Prepare path params in tight, single assignment
        _path = {'issueIdOrKey': issueIdOrKey, 'linkId': linkId}
        _query: Dict[str, Any] = {}

        # Prepare body with less branching, use dict unpacking for additional fields
        _body = {}
        if application is not None:
            _body['application'] = application
        if globalId is not None:
        ...
        if relationship is not None:
            _body['relationship'] = relationship
        if body_additional:
            _body |= body_additional  # Use dict union for fast update (Python 3.9+)
