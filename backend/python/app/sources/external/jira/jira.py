...
Path params:
  ...
  - itemId (str)"""
        ...
        # Use direct assignment, avoids redundant dict conversions
        _headers = headers if headers else {}
        _path = {'dashboardId': dashboardId, 'itemId': itemId}

        # The following are constant for this endpoint, construct once outside
        _query = {}
        _body = None
        rel_path = '/rest/api/3/dashboard/{dashboardId}/items/{itemId}/properties'
        url = self.base_url + _safe_format_url(rel_path, _path)
        # Inline _as_str_dict uses
        req = HTTPRequest(
            method='GET',
            url=url,
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # For single-use, direct dict comprehension is optimal
