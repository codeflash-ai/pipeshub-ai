...
Path params:
  - id (str)"""
        ...
        req = HTTPRequest(
            method='GET',
            url=url,
            headers=_fast_as_str_dict(headers),
            path_params={'id': str(id)},
            query_params={},  # always empty, avoid extra function call
            body=None,
        )
        resp = await self._client.execute(req)
        return resp
...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    # Only create mapping if there are params
    if not params:
        return template
    try:
...
def _fast_as_str_dict(d: Optional[Dict[str, Any]]) -> Dict[str, str]:
