...
_REL_PATH_AVATAR = '/rest/api/3/universal_avatar/view/type/{type}'
...
Query params:
  ...
  - format (str, optional)"""
        ...
        # Avoid unnecessary dict construction if headers is None or empty
        ...
        url = self.base_url + _safe_format_url(_REL_PATH_AVATAR, _path)
        # Avoid redundant conversion for trivial dict cases
        as_str_headers = _as_str_dict(_headers) if _headers else {}
        as_str_path = _as_str_dict(_path)  # always has one entry
        as_str_query = _as_str_dict(_query) if _query else {}
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    """Returns a dict with all keys and values as strings."""
    # Fast-path: avoid work if d already satisfies requirements
    if not d:
        return {}
    # Minor optimization: if already str:str, skip _serialize_value
    if all(isinstance(k, str) and isinstance(v, str) for k, v in d.items()):
        return d  # assumes no mutation of input downstream
