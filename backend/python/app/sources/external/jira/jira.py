...
Query params:
  ...
  - username (str, optional)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        _headers = headers if headers is not None else {}
        # Build _query dictionary efficiently
        _query = {}
        if accountId is not None:
            _query['accountId'] = accountId
        ...
        if username is not None:
            _query['username'] = username
        url = self.base_url + _safe_format_url('/rest/api/3/user/properties', {})
        # Only convert if necessary; if empty skip conversion
        as_str_dict = _as_str_dict
        headers_str = as_str_dict(_headers) if _headers else {}
        query_str = as_str_dict(_query) if _query else {}
        # _path is always empty; path_params can pass as {}, not need conversion
        req = HTTPRequest(
            method='GET',
            url=url,
            headers=headers_str,
