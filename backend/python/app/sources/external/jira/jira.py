...
Path params:
  - idOrKey (str)"""
        ...
        # Fast path for very small dicts and fixed path/query usage
        _path = {'idOrKey': idOrKey}
        _query = {}
        _body = None

        # Only clone and stringify headers if actually present
        if headers:
        ...
        # Inline as_str_dict for very small dicts
        req = HTTPRequest(
            method='GET',
            url=url,
            headers=_headers,
            path_params={'idOrKey': _serialize_value(idOrKey)},
...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    try:
        # Move _SafeDict outside the format_map call to avoid redundant instantiation
        safe_dict = _SafeDict(params)
