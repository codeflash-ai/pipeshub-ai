...
Query params:
  ...
  - maxResults (int, optional)"""
        ...
        # Path always only has fieldKey, use direct literal to avoid dict creation overhead
        ...
        try:
            url = f'{self.base_url}{rel_path.format(fieldKey=fieldKey)}'
        except Exception:
            url = self.base_url + rel_path  # Fallback is safe
        # Inline _as_str_dict for minor boost
        req = HTTPRequest(
            method='GET',
            url=url,
