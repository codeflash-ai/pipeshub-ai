...
Body (application/json) fields:
  - columns (list[str], optional)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Slightly faster copy for headers
        _headers: Dict[str, Any] = headers.copy() if headers is not None else {}
        _headers.setdefault('Content-Type', 'application/json')
        # avoid unnecessary dict instantiations
        _path_id = str(id)
        rel_path = '/rest/api/3/filter/{id}/columns'
        url = self.base_url + rel_path.format(id=_path_id)
        _body: Dict[str, Any] = {}
        ...
        req = HTTPRequest(
            ...
            url=url,
            headers=_as_str_dict(_headers),
            path_params={'id': _path_id},        # already a {str: str}
            query_params={},                     # always empty in this call
            body=_body,
        )
        resp = await self._client.execute(req)
        return resp
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Fast path: avoid calling if already str
    return {
        (k if isinstance(k, str) else str(k)):
            _serialize_value_fast(v)
        for k, v in d.items()
    }

def _serialize_value_fast(v: Union[bool, str, int, float, list, tuple, set, None]) -> str:
    # Fast path, always returns str
    if v is None:
        return ''
    # Fastest type checks: lists/tuples/sets
    if isinstance(v, (list, tuple, set)):
        # Avoid generator overhead for common case (lists of str/int)
        return ','.join(_to_bool_str_fast(x) for x in v)
    return _to_bool_str_fast(v)

def _to_bool_str_fast(v: Any) -> str:
    # Mirrors the likely implementation of the used _to_bool_str
    if v is True:
        return "true"
    elif v is False:
        return "false"
