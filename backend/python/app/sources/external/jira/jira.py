...
Path params:
  - id (str)"""
        ...
        if headers:
            ...
            str_headers = {}
        str_path = {'id': _serialize_value(id)}
        str_query = {}
        rel_path = '/rest/api/3/version/{id}/unresolvedIssueCount'
        # Fast path for single-item path dict
        url = self.base_url + _fast_format_url(rel_path, str_path)
        req = HTTPRequest(
            method='GET',
            url=url,
            headers=str_headers,
...
def _fast_format_url(template: str, params: Dict[str, object]) -> str:
    # For fast single key replacement, avoid full format_map overhead
    if not params:
        return template
    try:
        # Use str.format for tiny dicts with well-known keys
        return template.format(**params)
    except Exception:
        return template

def _to_bool_str(v: Any) -> str:
    # Helper per original read-only code semantics
    if isinstance(v, bool):
        return 'true' if v else 'false'
    return str(v)
