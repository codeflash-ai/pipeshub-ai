...
Query params:
  ...
  - maxResults (int, optional)"""
        ...
        # Optimize URL formatting for empty _path (hot-path shortcut)
        if not _path:
            url = self.base_url + rel_path
        else:
            url = self.base_url + _safe_format_url(rel_path, _path)

        # Use fast-path for empty dicts
        req = HTTPRequest(
            method='GET', 
            url=url, 
...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    # Fast-path: if no params, return template
    if not params:
        return template
    try:
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Fast-path for empty dict, which is common
    if not d:
        return {}
