...
Path params:
  - linkId (str)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Use fast-path for headers, path, and query. 
        _headers = headers if headers and isinstance(headers, dict) else {}
        # path is always just {'linkId': linkId}; no cast needed
        _path = {'linkId': linkId}
        # query is always empty
        _query = {}
        _body = None
        rel_path = '/rest/api/3/issueLink/{linkId}'
        url = f"{self.base_url}/rest/api/3/issueLink/{linkId}"  # fast f-string instead of format_map for this static path
        req = HTTPRequest(
            method='GET',
            url=url,
            headers=_as_str_dict(_headers),
            path_params={'linkId': str(linkId)},    # direct conversion, no dict-comp needed for single item
            query_params={},                        # always empty; skip conversion and allocation
            body=_body,
        )
        resp = await self._client.execute(req)
        return resp
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    ...
    # Short-circuit for empty dict or already correct mapping
    if not d:
        return {}
    # If already str to str, just return
