...
Query params:
  ...
  - level (int, optional)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Use fast empty-dict path
        _headers = headers if headers else _EMPTY_OBJ_DICT
        _path = _EMPTY_OBJ_DICT  # This endpoint never uses path params, always empty
        _query = {'projectId': projectId}
        if level is not None:
            _query['level'] = level
        _body = None
        rel_path = '/rest/api/3/issuetype/project'
        # Fast path for static rel_path and always-empty path params
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    """Convert dict values and keys to strings as required, but optimize for empty case."""
    if not d:
