...
Query params:
  - name (str, required)"""
        ...
        url = self.base_url + _safe_format_url(rel_path, _path)
        req = HTTPRequest(method='GET', url=url, headers=_as_str_dict(_headers) if _headers else {}, path_params=_as_str_dict(_path) if _path else {}, query_params=_as_str_dict(_query), body=_body)
        resp = await self._client.execute(req)
        return resp
