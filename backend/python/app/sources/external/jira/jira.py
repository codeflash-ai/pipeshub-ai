...
Query params:
  - deleteSubtasks (str, optional)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Avoid unnecessary copying, do conversion only once
        _headers = headers if headers is not None else {}
        _path = {'issueIdOrKey': issueIdOrKey}
        # Since _query might be empty most calls, optimize for that case
        ...
        # Avoid string concatenation if safe_format returns unmodified template. Move rstrip out of path-building.
        formatted_path = _safe_format_url(rel_path, _path)
        url = f'{self.base_url}{formatted_path}'
        # Precompute string dicts only once, reuse
        str_headers = _as_str_dict(_headers)
        str_path = { 'issueIdOrKey': issueIdOrKey }
        str_query = _query if not _query else _as_str_dict(_query)
        req = HTTPRequest(
            method='DELETE',
            url=url,
            headers=str_headers,
...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    if not params:
        return template
    try:
        # Avoid unnecessary abstraction overhead if params is already a dict
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    if not d:
        return {}
    # Use list comprehension for fast path in CPython
