...
HTTP GET /rest/api/3/issuesecurityschemes"""
        ...
        if not _path:
            ...
            url = self.base_url + _safe_format_url(rel_path, _path)
        req = HTTPRequest(method='GET', url=url, headers=_as_str_dict(_headers) if _headers else {}, path_params={} if not _path else _as_str_dict(_path), query_params={} if not _query else _as_str_dict(_query), body=_body)
        resp = await self._client.execute(req)
        return resp
...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    # Fast path: If 'params' is empty, avoid format_map entirely.
    if not params:
        return template
    try:
