...
_REL_PATH = '/rest/api/3/project/{projectKeyOrId}/permissionscheme'
...
Query params:
  - expand (str, optional)"""
        ...
        # Use headers dict directly if already dict, else create new dict. No need for unnecessary copy when headers is None.
        ...
        url = self.base_url + _fast_format_url(_REL_PATH, _path)
        # Optimized fast-path for _as_str_dict
        req = HTTPRequest(
            method='GET',
            url=url,
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Fast-path if d is empty (very common case for _query, _headers)
    if not d:
        return {}
    # Fast-path for single-item dict (usual for _path and _query)
    if len(d) == 1:
        k, v = next(iter(d.items()))
        key = k if isinstance(k, str) else str(k)
        value = _serialize_value(v)
        return {key: value}
    # General path: skip str() for string keys, avoid full generator overhead
    return {k if isinstance(k, str) else str(k): _serialize_value(v) for (k, v) in d.items()}


def _fast_format_url(template: str, params: Dict[str, object]) -> str:
    """Optimized for known rel_path with only {projectKeyOrId} placeholder."""
    # Fast path for exactly our endpoint: only one param expected.
    try:
        value = params['projectKeyOrId']
        # It's always a str per function contract, but ensure.
        return template.replace('{projectKeyOrId}', str(value))
    except Exception:
        return _safe_format_url(template, params)
