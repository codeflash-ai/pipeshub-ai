...
Path params:
  - planId (int)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Combine headers dict and convert to str dict only once
        _headers = _as_str_dict(headers) if headers else {}
        # Small dict for path param, convert inline
        _path = {'planId': planId}
        _path_str = {'planId': _serialize_value(planId)}
        # Query params is always empty
