...
# Used constant empty dicts to avoid per-call allocation for hot paths
...
HTTP GET /rest/api/3/jql/autocompletedata"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        _headers: Dict[str, Any] = headers.copy() if headers else EMPTY_DICT
        # Below path/query dicts are always empty -> use constant
        _path: Dict[str, Any] = EMPTY_DICT
        _query: Dict[str, Any] = EMPTY_DICT
        _body = None
        rel_path = '/rest/api/3/jql/autocompletedata'
        url = self.base_url + _safe_format_url(rel_path, _path)
        # Only convert _headers if it's not empty; _path/_query always empty and always returned as {} -> '{}' by _as_str_dict
        req = HTTPRequest(
            method='GET',
            url=url,
            headers=_as_str_dict(_headers) if _headers else EMPTY_DICT,
            path_params=EMPTY_DICT,
            query_params=EMPTY_DICT,
            body=_body
        )
        resp = await self._client.execute(req)
        return resp
...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    # If params is empty, short-circuit: format_map would not change anything
    if not params:
        return template
    try:
        _SafeDict_ = _SafeDict  # local ref for speed
        return template.format_map(_SafeDict_(params))
    except Exception:
        return template
...
def _to_bool_str(x: Any) -> str:
    # Inlined per read-only reference module logic. Avoid import penalty for hot call.
    if isinstance(x, bool):
        return str(x).lower()
    return str(x)

def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Empty dict path hot, return without dictcomp
    if not d:
        return EMPTY_DICT
    str_ = str
    _serialize_value_ = _serialize_value
    # Use generator and then dict constructor for slight perf gain over dictcomp, though in Py3.10 dictcomp is C-speed.
    return {str_(k): _serialize_value_(v) for (k, v) in d.items()}
