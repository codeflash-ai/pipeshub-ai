...
Body (application/json) fields:
  ...
  - properties (Dict[str, Any], optional)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Optimize header construction and avoid unnecessary copies when possible
        if headers is None:
        ...
        # These are always empty, avoid allocating/calling conversions for them
        _path = {}
        _query = {}
        ...
        # rel_path and _path are always constant, so avoid repeated formatting
        url = self.base_url + '/rest/api/3/issue/properties'

        # Only convert dict to str dict if non-empty (these are always empty here, so bypass)
        path_params_str = {}
        query_params_str = {}
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    ...
    if not d:  # Short circuit for common case of empty dict
        return {}
