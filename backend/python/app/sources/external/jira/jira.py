...
# Preallocated empty dicts for efficiency
...
Body (application/json) fields:
  - mappings (list[Dict[str, Any]], required)"""
        ...
        # Use direct assignment for single-path param (no dict overhead)
        _path = {'id': id}

        # Use preallocated empty dict
        _query = EMPTY_DICT
        _body = {'mappings': mappings}
        ...
        req = HTTPRequest(
            ...
            headers=_as_str_dict(_headers),
            path_params=_as_str_dict(_path),
            query_params=EMPTY_STR_DICT, # always empty here
            body=_body,
        )
        resp = await self._client.execute(req)
        return resp
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    ...
    # Fast path: dict is empty, return shared empty dict
    if not d:
        return EMPTY_STR_DICT
    # For single-key dict, avoid dict comprehension overhead
    if len(d) == 1:
        k, v = next(iter(d.items()))
        return {str(k): _serialize_value(v)}
    # Both keys and values may be non-string, so avoid alloc if already correct
