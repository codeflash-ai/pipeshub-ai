...
Path params:
  - id (str)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Optimize: avoid copying unless needed
        _headers: Dict[str, Any] = headers if headers is not None else {}
        # Faster: use direct dict assignment
        _path = {'id': id}
        rel_path = '/rest/api/3/attachment/{id}/expand/human'
        # Use fast string formatting since path is simple and rel_path always expects 'id'
        url = self.base_url + rel_path.format(id=id)
        # For small dicts, inline str conversion for path/query
        req = HTTPRequest(
            method='GET',
            url=url,
            headers=_as_str_dict(_headers),
            path_params={'id': str(id)},  # _path contains only id
            query_params={},              # _query is always {}
            body=None,
        )
        resp = await self._client.execute(req)
        return resp
