...
# Singletons to prevent repeated empty dict allocations
...
HTTP GET /rest/api/3/dashboard/gadgets"""
        ...
        # Avoid unnecessary dict allocation/flattening for empty cases (very common path for this endpoint)
        if headers:
            _headers: Dict[str, Any] = dict(headers)
            headers_dict = _as_str_dict(_headers)
        else:
            _headers = _EMPTY_DICT
            headers_dict = _EMPTY_STR_DICT
        # _path/_query always empty here
        _path: Dict[str, Any] = _EMPTY_DICT
        _query: Dict[str, Any] = _EMPTY_DICT
        _body = None
        rel_path = '/rest/api/3/dashboard/gadgets'
        # rel_path is constant, _path is always empty; skip format_map entirely
