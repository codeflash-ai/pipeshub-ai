...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    # Fast path: if params is empty, return template immediately as no formatting needed
    if not params:
        return template
    try:
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    ...
    # Fast path: if input is empty, just return {}
    if not d:
        return {}
    # Avoid unnecessary conversion if already correct
    # (optimization: skip dict comprehension for empty-case already handled)
