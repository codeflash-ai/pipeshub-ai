...
Path params:
  - key (str)"""
        ...
        # Optimize: use {} if headers is None, use headers.copy() otherwise (shallow) - avoid an extra dict allocation
        _headers: Dict[str, Any] = headers.copy() if headers is not None else {}
        _path = {'key': key}
        # Optimize: _query is always empty, use shared singleton
        _query = _EMPTY_STR_DICT
        _body = None
        rel_path = '/rest/api/3/applicationrole/{key}'
        url = self.base_url + _safe_format_url(rel_path, _path)

        # Micro-opt: pattern-match fast path for empty/one-kv dicts
        hdrs = _as_str_dict(_headers) if _headers else _EMPTY_STR_DICT
        path_params = _as_str_dict(_path)
        request = HTTPRequest(
            method='GET',
            url=url,
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    ...
    # Fast path: if already all str->str, return as is
...
def _to_bool_str(v: Any) -> str:
    # Helper used by _serialize_value from original referenced module
    if isinstance(v, bool):
        return "true" if v else "false"
    return str(v)
