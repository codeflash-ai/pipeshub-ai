...
Body (application/json) fields:
  ...
  - additionalProperties allowed (pass via body_additional)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Combine headers efficiently and use setdefault in a single pass
        _headers: Dict[str, Any] = dict(headers) if headers else {}
        if 'Content-Type' not in _headers:
            _headers['Content-Type'] = 'application/json'

        # Compose parameters with minimal copying
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    ...
    # Fast path: avoid unnecessary dict copying if already correct type
    if not d:
        return {}
