...
# Shared singleton for empty dicts (saves allocations)
...
Query params:
  - expand (str, optional)"""
        ...
        # Use fast-path for empty headers, path, and query dicts
        _headers = headers if headers is not None and headers else _EMPTY_STR_DICT
        _path = _EMPTY_STR_DICT
        if expand is not None:
            _query = {'expand': expand}
        else:
        ...
        req = HTTPRequest(
            ...
            url=url,
            headers=_as_str_dict(_headers),
            path_params=_EMPTY_STR_DICT,      # Already string dict for empty
            query_params=_as_str_dict(_query),
            body=_body,
        )
        resp = await self._client.execute(req)
...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    if not params:  # Fast-path for empty params
        return template
    try:
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Fast-path: empty dict, return shared instance
    if not d:
