...
_REL_PATH = '/rest/api/3/fieldconfigurationscheme/project'
...
Query params:
  ...
  - projectId (list[int], required)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Avoid unnecessary dict copy if headers is None or already empty
        if headers:
            _headers = {str(k): _serialize_value(v) for k, v in headers.items()}
        else:
            _headers = {}
        _path = {}
        # Efficient dict construction
        if startAt is not None or maxResults is not None:
            _query = {}
            if startAt is not None:
                _query['startAt'] = startAt
            if maxResults is not None:
        ...
        # Pre-stringify path and query once (as per HTTPRequest expectations)
        req = HTTPRequest(
            method='GET',
            url=url,
            headers=_headers,
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    ...
    if not d:
        return {}
    # Instead of always calling str and _serialize_value, shortcut when value is already a string
    return {str(k): v if isinstance(v, str) else _serialize_value(v) for k, v in d.items()}
