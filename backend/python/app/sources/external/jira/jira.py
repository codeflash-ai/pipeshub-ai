...
class JiraDataSource:
    ...
    def __init__(self, client: JiraClient) -> None:
        ...
        try:
            self.base_url = _client.get_base_url().rstrip('/')
...
Body (application/json) fields:
  ...
  - additionalProperties allowed (pass via body_additional)"""
        ...
        # Fast path: _path is always empty for this endpoint, avoid unnecessary format logic
        if not _path:
        ...
        resp = await _client.execute(req)
        return resp
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Avoid unnecessary work if dict is empty
    if not d:
        return {}
    # Inline the common path - works slightly faster than comprehension for small dicts
    result = {}
    for k, v in d.items():
