...
Body: application/json (str)"""
        ...
        # Inline headers construction and avoid dict copy when possible
        if headers is None:
        ...
        # Compose url with inline path params to avoid intermediate dicts
        rel_path = '/rest/api/3/project/{projectIdOrKey}/properties/{propertyKey}'
        _path = {'projectIdOrKey': projectIdOrKey, 'propertyKey': propertyKey}
        url = f"{self.base_url}{_safe_format_url(rel_path, _path)}"

        # Inline as_str_dict usage for known-size dicts
        req = HTTPRequest(
            method='PUT',
            url=url,
            headers=_as_str_dict(_headers) if _headers else {},
            path_params={'projectIdOrKey': str(projectIdOrKey), 'propertyKey': str(propertyKey)},
...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    # Fast-path: if no '{' in template, skip formatting
    if '{' not in template:
        return template
    try:
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    if not d:
        return {}
    # Micro-optimize for small dicts: avoid function overhead for trivial cases
    items = d.items()
    if len(d) == 2:
        (k1, v1), (k2, v2) = items
        return {str(k1): _serialize_value(v1), str(k2): _serialize_value(v2)}
    elif len(d) == 1:
        (k, v), = items
