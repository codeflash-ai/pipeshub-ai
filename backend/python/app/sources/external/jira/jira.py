...
Body (application/json) fields:
  ...
  - additionalProperties allowed (pass via body_additional)"""
        ...
        # dict(headers or {}) is faster than dict(headers) if None
        _headers: Dict[str, Any] = headers.copy() if headers else {}
        # setdefault only if not present
        if 'Content-Type' not in _headers:
            _headers['Content-Type'] = 'application/json'
        # Empty _path and _query are used for formatting and requests, avoid re-allocation
        _path: Dict[str, Any] = {}
        _query: Dict[str, Any] = {}
        # Build _body with only present values
...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    # For static endpoint, quickly return template
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    ...
    if not d:
        return {}
    # Avoids unnecessary allocation and function calls for already string values
    out = {}
    for k, v in d.items():
