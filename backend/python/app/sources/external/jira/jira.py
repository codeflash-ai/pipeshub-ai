...
Path params:
  - id (str)"""
        ...
        url = self.base_url + _safe_format_url(rel_path, _path)
        # Avoid repeated conversions when dicts are empty (as is always the case with _query)
        req = HTTPRequest(
            method='DELETE',
            url=url,
            headers=_as_str_dict(_headers),
            path_params=_as_str_dict(_path),
            query_params={} if not _query else _as_str_dict(_query),  # Avoid generator when always empty
            body=_body)
        resp = await self._client.execute(req)
        return resp
...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    # Fast path for known, simple use in delete_dashboard
    if template == '/rest/api/3/dashboard/{id}' and len(params) == 1 and 'id' in params:
        # Only substitute {id}, and avoids .format_map overhead
        return f'/rest/api/3/dashboard/{params["id"]}'
    try:
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    if not d:
        return {}  # Avoid unnecessary generator/comprehension for empty dicts
