...
_REL_PATH = '/rest/api/3/issuetype/{issueTypeId}/properties/{propertyKey}'
...
Path params:
  ...
  - propertyKey (str)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Only process headers if present to avoid converting empty dict every time
        if headers:
            _headers: Dict[str, Any] = dict(headers)
            str_headers = _as_str_dict(_headers)
        else:
            str_headers = _EMPTY_STR_DICT

        # Since _path is always the same two keys, we can avoid dict allocation and just use a tuple
        url = self.base_url + _format_issue_type_property_url(issueTypeId, propertyKey)
        req = HTTPRequest(
            method='GET',
            url=url,
            headers=str_headers,
            path_params={'issueTypeId': issueTypeId, 'propertyKey': propertyKey},
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    ...
    # Return static empty dict for empty input
    if not d:
...
# ---- Helpers used by generated methods ----
def _format_issue_type_property_url(issueTypeId: str, propertyKey: str) -> str:
    # Specialized formatting for hot-path with only 2 fixed keys
    return _REL_PATH.format(issueTypeId=issueTypeId, propertyKey=propertyKey)
