...
Path params:
  - taskId (str)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Fast path for empty input dicts, avoid unnecessary allocations
        ...
        url = self.base_url + _safe_format_url(rel_path, _path)
        # Use specialized fast path for small dicts in _as_str_dict
        req = HTTPRequest(
            method='GET',
            url=url,
            headers=_as_str_dict(_headers),
            path_params={'taskId': _serialize_value(taskId)},  # direct, faster for single key
...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    # Fast path: if only one param, use .replace for direct performance
    if len(params) == 1:
        # detect placeholder
        k, v = next(iter(params.items()))
        placeholder = '{' + k + '}'
        if placeholder in template:
            return template.replace(placeholder, str(v))
    try:
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Fast path for empty dict usage in query/headers
    if not d:
        return {}
    elif len(d) == 1:
