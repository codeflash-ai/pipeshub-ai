...
HTTP GET /rest/api/3/attachment/meta"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Only process headers/path/query if provided or non-empty
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Use a generator expression for more memory efficiency during conversion
    return {str(k): _serialize_value(v) for k, v in d.items() if d}
