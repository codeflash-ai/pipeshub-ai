...
Body (application/json) fields:
  ...
  - expression (str, required)"""
        ...
        # Only create dicts if necessary
        _path: Dict[str, Any] = {}
        _query: Dict[str, Any] = {}
        if expand is not None:
            _query['expand'] = expand
        _body: Dict[str, Any] = {'expression': expression}
        if context is not None:
            _body['context'] = context
        rel_path = '/rest/api/3/expression/evaluate'

        # Fast-path for _safe_format_url if _path is empty
        url = (
            self.base_url + rel_path
            if not _path
            else self.base_url + _safe_format_url(rel_path, _path)
        )

        # Avoid unnecessary _as_str_dict on empty dicts
        req = HTTPRequest(
            method='POST',
            url=url,
