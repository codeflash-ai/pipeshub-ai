...
Body (application/json) fields:
  ...
  - screens (Dict[str, Any], required)"""
        ...
        _headers: Dict[str, Any] = dict(headers or {})
        _headers.setdefault('Content-Type', 'application/json')
        # Avoid unnecessary formatting and conversion for empty dicts
        rel_path = '/rest/api/3/screenscheme'
        url = self.base_url + rel_path
        # Accumulate body values as a batch for minor efficiency
        _body: Dict[str, Any] = {'name': name, 'screens': screens}
        if description is not None:
            _body['description'] = description
        req = HTTPRequest(
            method='POST',
            ...
            path_params={},  # No need for _as_str_dict for empty dicts
            query_params={},  # No need for _as_str_dict for empty dicts
            body=_body
        )
        resp = await self._client.execute(req)
        return resp
