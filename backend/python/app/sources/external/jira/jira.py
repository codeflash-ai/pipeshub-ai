...
Query params:
  - redirect (bool, optional)"""
        ...
        if client is None:
            raise ValueError('HTTP client is not initialized')
        # Avoid dict copy if headers is None or already a dict
        if headers:
            _headers = {str(k): _serialize_value(v) for k, v in headers.items()}
        else:
            _headers = {}
        # Inline _as_str_dict and avoid creating a dict just to stringify
        _path = {'id': id}
        _path_str = {'id': _serialize_value(id)}
        # Only add 'redirect' if needed
        if redirect is not None:
            _query = {'redirect': redirect}
            _query_str = {'redirect': _serialize_value(redirect)}
        else:
            _query = {}
            _query_str = {}
        _body = None
