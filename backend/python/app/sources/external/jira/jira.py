...
def _serialize_value(v: Union[bool, str, int, float, list, tuple, set, None]) -> str:
    ...
    if isinstance(v, (list, tuple, set)):
        # Use map for better performance than generator/tuple on simple function calls
        return ','.join(map(_to_bool_str, v))
    return _to_bool_str(v)

def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Minor optimization: Use dictionary comprehension directly; performance is already high
