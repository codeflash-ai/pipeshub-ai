...
Path params:
  ...
  - propertyKey (str)"""
        ...
        # Optimize by avoiding unnecessary conversion for simple dicts
        if headers is None:
            _headers = {}
        elif isinstance(headers, dict) and all(isinstance(k, str) and isinstance(v, str) for k, v in headers.items()):
            _headers = headers
        else:
            _headers = dict(headers)

        _path = {'issueTypeId': issueTypeId, 'propertyKey': propertyKey}
        _query = {}  # always empty, so optimized handling
        _body = None
        ...
        req = HTTPRequest(
            ...
            query_params={},  # always empty, skip processing
            body=_body,
        )
        resp = await self._client.execute(req)
        return resp
...
def _fast_as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # If already dict[str,str], return as is; else, convert
    if d and all(isinstance(k, str) and isinstance(v, str) for k, v in d.items()):
        return d
    return {str(k): _serialize_value(v) for k, v in d.items()}

def _to_bool_str(x: Any) -> str:
    # Replicates upstream expected serialization behavior
