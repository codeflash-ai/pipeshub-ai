...
Query params:
  ...
  - keyFilter (str, optional)"""
        ...
        # Direct construction of query dict using tuple pairing
        _query = {}
        for param_name, param_value in (('key', key), ('permissionLevel', permissionLevel), ('keyFilter', keyFilter)):
        ...
        # Use optimized _as_str_dict for empty dicts
        req = HTTPRequest(
            method='GET',
            url=url,
            headers=_as_str_dict(_headers),
...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    # Fast path: do not format if no params
    if not params:
        return template
    try:
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    ...
    if not d:
        return {}
    # If all keys and values are already str, avoid dict comprehension
    if all(isinstance(k, str) and isinstance(v, str) for k, v in d.items()):
        return d  # safely return the same mapping
