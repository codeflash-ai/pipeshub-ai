...
Path params:
  ...
  - contextId (int)"""
        ...
        # Minor optimization: avoid copying dict if headers is None or already str->str
        # Only convert if header values are not str
        _headers: Dict[str, Any] = headers if headers else {}
        if _headers:
            # Only serialize if any value in _headers is not a str
            if not all(isinstance(v, str) for v in _headers.values()):
                headers_str = {str(k): _serialize_value(v) for k, v in _headers.items()}
            else:
                headers_str = {str(k): v for k, v in _headers.items()}
        else:
            headers_str = {}

        # Known path keys, minimizing allocations
        path_str = {
            'fieldId': str(fieldId),
            'contextId': str(contextId),
        }

        # Query and body are always empty/None here
        query_str = {}

        rel_path = '/rest/api/3/field/{fieldId}/context/{contextId}'
        url = self.base_url + _safe_format_url(rel_path, path_str)
