...
Path params:
  - issueTypeSchemeId (int)"""
        ...
        url = self.base_url + rel_path.replace('{issueTypeSchemeId}', str(issueTypeSchemeId))
        req = HTTPRequest(method='DELETE', url=url, headers=_as_str_dict(_headers) if _headers else {}, path_params={'issueTypeSchemeId': str(issueTypeSchemeId)}, query_params={}, body=_body)
        resp = await self._client.execute(req)
        return resp
