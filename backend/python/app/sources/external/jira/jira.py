...
Body (application/json) fields:
  ...
  - transitions (list[Dict[str, Any]], required)"""
        ...
        # Minimize allocations and redundant dict copying
        if headers is None:
            _headers: Dict[str, Any] = {'Content-Type': 'application/json'}
        else:
            # single allocation; no dict(headers)
        ...
        # Minimize dict operations by batched update
        _body: Dict[str, Any] = {
            'name': name,
            'statuses': statuses,
        ...
        if description is not None:
            _body['description'] = description
        rel_path = '/rest/api/3/workflow'
        # Using f-string is marginally faster than string concat for short strings
        url = f"{self.base_url}{_safe_format_url(rel_path, _path)}"

        # Avoid redundant dict comprehensions for _as_str_dict on empty dicts
        req = HTTPRequest(
            method='POST',
            url=url,
...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    # Fast path if params is empty to avoid creating _SafeDict
    if not params:
        return template
    try:
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Early return on empty dict avoids generator overhead
    if not d:
        return {}
