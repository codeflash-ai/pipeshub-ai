...
Body: application/json (str)"""
        ...
        # Fast path: reuse headers if possible, mutate in place if they're not shared
        if headers is not None:
            _headers: Dict[str, Any] = headers if 'Content-Type' in headers else dict(headers)
            _headers.setdefault('Content-Type', 'application/json')
        else:
            # Avoid allocation of dict with every call
            _headers = {'Content-Type': 'application/json'}

        # Path variable is always provided
        _path = {'issueIdOrKey': issueIdOrKey}
        _query = {}  # Don't reallocate empty dict every call
        _body = body
        rel_path = '/rest/api/3/issue/{issueIdOrKey}/watchers'
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Fast path for empty dict, return shared singleton as no mutation
    if not d:
        return {}
    # Minimize dict allocations, try to avoid unnecessary conversions
    items = d.items()
    for k, v in items:
        if not isinstance(k, str) or not isinstance(v, str):
            # Need conversion
            return {str(k): _serialize_value(v) for k, v in items}
    # If all items already string, just copy/return-as-is for shallow-dict
    return dict(items)
