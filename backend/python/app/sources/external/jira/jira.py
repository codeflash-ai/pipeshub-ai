...
Path params:
  ...
  - entityId (str)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # If headers==None or Dict[str, Any], do a shallow copy only if needed
        _headers = headers if headers is not None else {}
        _path = {'type': type, 'entityId': entityId}
        # _query and _body set once only
        _query = {}
        _body = None
        rel_path = '/rest/api/3/universal_avatar/type/{type}/owner/{entityId}'
        url = self.base_url + _safe_format_url(rel_path, _path)
        # Do conversion once where needed; avoid recreating dicts
        req = HTTPRequest(
            method='GET',
            url=url,
            headers=_as_str_dict(_headers),
            path_params=_as_str_dict(_path),  # _path is tiny
            query_params=_as_str_dict(_query),  # _query is always empty
            body=_body
        )
        resp = await self._client.execute(req)
        return resp
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Short-circuit if d is empty or already str->str
    if not d:
        return {}
    if all(isinstance(k, str) and isinstance(v, str) for k, v in d.items()):
        # If already Dict[str, str], copy directly for safety
        return dict(d)
    # Otherwise, conversion as usual
