...
Body (application/json) fields:
  ...
  - self (str, optional)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Avoid unnecessary dict allocation: merge only when headers are given
        if headers:
        ...
        if self_ is not None:
            _body['self'] = self_
        rel_path = '/rest/api/3/issueLinkType'
        # _path always empty here; skip _safe_format_url for performance
        ...
        req = HTTPRequest(
            ...
            url=url,
            headers=_as_str_dict(_headers),
            path_params={},         # skip conversion for static empty dict
            query_params={},        # skip conversion for static empty dict
            body=_body,
        )
        resp = await self._client.execute(req)
        return resp
