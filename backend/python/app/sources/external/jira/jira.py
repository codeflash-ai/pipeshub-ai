...
Path params:
  - key (str)"""
        ...
        # Since path param is always 'key', we can use f-string directly
        url = f"{self.base_url}{rel_path.replace('{key}', str(key))}"

        # Convert headers only if provided, otherwise use empty dict
        if headers:
            _headers = {str(k): _serialize_value(v) for (k, v) in headers.items()}
        else:
            _headers = {}

        # Single path param - this is always {'key': str(key)}
        _str_path = {'key': str(key)}
        # Query and body are always empty for this endpoint
        req = HTTPRequest(
            method='GET',
            url=url,
            headers=_headers,
