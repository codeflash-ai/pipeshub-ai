...
Path params:
  - id (str)"""
        ...
        # Avoid redundancy; build directly, minimize dict copy and function calls
        url = self.base_url + _safe_format_url(rel_path, {'id': id})

        # Avoid extra dict copying for small & known-dim dicts
        headers_dict = dict(headers) if headers else {}

        # Use precomputed _as_str_dict for each -- do not create an explicit _path, _query, _body
        req = HTTPRequest(
            method='DELETE',
            url=url,
