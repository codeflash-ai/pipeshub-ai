...
Body (application/json) fields:
  ...
  - name (str, optional)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Use local mutation and shared constant for empty dicts to save allocation; setdefault is inplace and not a performance concern here.
        if headers:
        ...
        if name is not None:
            _body['name'] = name
        rel_path = '/rest/api/3/role'
        # Only need to format_url if _path (params) are present. If empty, output is the template.
        url = self.base_url + _safe_format_url(rel_path, EMPTY_DICT)
        # If _path and _query always empty here, no need to create new dicts every call.
        req = HTTPRequest(
            method='POST', 
            url=url, 
...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    # If params empty, immediately return template (avoids SafeDict/format_map cost for hotpath)
    if not params:
        return template
    try:
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Shortcut for empty dict improves performance on hotpath
    if not d:
        return {}
