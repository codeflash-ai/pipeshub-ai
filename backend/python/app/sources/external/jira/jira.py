...
Query params:
  ...
  - projectId (list[str], optional)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Fast dict merge: avoid unnecessary copy if headers is None or empty
        _headers: Dict[str, Any] = headers.copy() if headers else {}
        # _path is always empty; do not allocate each time
        _path: Dict[str, Any] = {}
        # Build _query only if params are set
        _query = {}
        if startAt is not None:
            _query['startAt'] = startAt
        if maxResults is not None:
        ...
        # Directly compose the url as formatting will be a no-op with empty path
        url = f"{self.base_url}{rel_path}"
        # Convert only non-empty dicts for path/query/headers
        req = HTTPRequest(
            method='GET',
            url=url,
            headers=_as_str_dict(_headers) if _headers else {},
            path_params={},  # path is always empty
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Avoids unnecessary work for empty dict
    if not d:
        return {}
