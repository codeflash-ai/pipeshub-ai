...
Path params:
  ...
  - permissionId (int)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Avoid unnecessary dict constructor overhead
        _headers: Dict[str, Any] = headers.copy() if headers is not None else {}
        # Direct use, avoid dict overhead since path is always identical
        ...
        # Only call _as_str_dict when necessary
        req = HTTPRequest(
            method='DELETE',
            url=url,
            headers=_as_str_dict(_headers),
            path_params={ 'id': str(id), 'permissionId': str(permissionId) },
            query_params={},  # No need to call _as_str_dict({}) for empty dict
            body=None
        )
        resp = await self._client.execute(req)
        return resp
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Fast path for empty dict; otherwise, run comprehension
    if not d:
        return {}
