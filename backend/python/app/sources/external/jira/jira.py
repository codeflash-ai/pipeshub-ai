...
class JiraDataSource:
    ...
    @codeflash_capture(
        function_name='JiraDataSource.__init__',
        tmp_dir_path='/tmp/codeflash_1j5ekrv2/test_return_values',
        tests_root='/home/ubuntu/work/repo/backend/python/tests',
        is_fto=True)
...
Path params:
  - projectIdOrKey (str)"""
        ...
        url = self.base_url + _safe_format_url(rel_path, _path)
        # Avoid re-computing empty dicts and conversions
        _query = {}
        _body = None
        req = HTTPRequest(
            method='GET',
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Manual loop for micro-optimization, avoid calling str() if key is already str
    out = {}
    for k, v in d.items():
