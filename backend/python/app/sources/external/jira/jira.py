...
Body (application/json) fields:
  ...
  - name (str, required)"""
        ...
        # Avoid unnecessary copying for default empty cases
        if headers:
        ...
        rel_path = '/rest/api/3/fieldconfigurationscheme'
        # Preallocate and skip empty cases for dict-to-str conversions
        _path: Dict[str, Any] = {}
        _query: Dict[str, Any] = {}
        url = self.base_url + _safe_format_url(rel_path, _path)

        # Avoid calling _as_str_dict for empty dictionaries
...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    # Skip format_map if params is empty, saves time on empty cases
    if not params:
        return template
    try:
