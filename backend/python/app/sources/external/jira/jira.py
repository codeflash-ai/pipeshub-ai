...
Query params:
  - expand (str, optional)"""
        ...
        path_str = {'id': str(id)}
        query_str = {'expand': _serialize_value(expand)} if expand is not None else {}

        req = HTTPRequest(method='GET', url=url, headers=headers_str, path_params=path_str, query_params=query_str, body=_body)
        resp = await self._client.execute(req)
        return resp
