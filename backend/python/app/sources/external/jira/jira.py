...
Query params:
  ...
  - keyFilter (str, optional)"""
        ...
        # Fast path for dict initialization, avoids redundant copies and allocations
        _headers = headers if headers is not None else {}
        _path = {}  # No path parameters for this endpoint

        # Build _query dict in a tight block
        _query = {}
        if key is not None:
            _query['key'] = key
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    if not d:
        return {}
    # For small dicts, list comprehension can be slightly faster; 
    # but for larger dicts, normal dict comprehension is C-optimized.
