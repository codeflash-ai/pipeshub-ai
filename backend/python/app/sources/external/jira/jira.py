...
Path params:
  - screenId (int)"""
        ...
        # Precompute str-dicts only when needed, avoid unnecessary construction
        _headers = headers if headers is not None else {}
        _path = {'screenId': screenId}
        # _query is always empty, skip unnecessary dict construction
        rel_path = '/rest/api/3/screens/{screenId}/availableFields'
        base_url = self.base_url

        url = base_url + _safe_format_url(rel_path, _path)

        # Lazily convert dicts only if non-empty for slight micro-optimization
        req = HTTPRequest(
            method='GET',
            url=url,
            headers=_as_str_dict(_headers) if _headers else {},
            path_params=_as_str_dict(_path),
            query_params={},  # _as_str_dict({}) always returns {}
            body=None
        )
        resp = await self._client.execute(req)
        return resp
