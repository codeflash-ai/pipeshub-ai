...
Body (application/json) fields:
  ...
  - position (str, optional)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Use fast-path for empty or None headers
        if headers:
        ...
        _path: Dict[str, Any] = {'id': id}
        _query: Dict[str, Any] = {}
        # Batch assignment for body
        if after is None and position is None:
            _body: Dict[str, Any] = {}
        elif after is not None and position is not None:
            _body: Dict[str, Any] = {'after': after, 'position': position}
        elif after is not None:
            _body: Dict[str, Any] = {'after': after}
        else:  # position is not None
            _body: Dict[str, Any] = {'position': position}
...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    # Avoid formatting for static templates
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Fast-path for empty dictionary
    if not d:
        return {}
    result = {}
    for k, v in d.items():
        # Avoid unnecessary function calls for strings
        if isinstance(v, str):
            result[str(k)] = v
        else:
