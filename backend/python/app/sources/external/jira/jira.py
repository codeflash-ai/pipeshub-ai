...
Body (application/json) fields:
  ...
  - searcherKey (str, optional)"""
        ...
        # Fast dict creation and inline str conversion for headers.
        ...
        # _path and _query are constant and small, so convert directly
        _path = {'fieldId': fieldId}
        _body = {}
        if description is not None:
            _body['description'] = description
        if name is not None:
        ...
        url = self.base_url + _safe_format_url(rel_path, _path)
        # Convert headers, path, query dicts to str only as needed
        str_headers = {str(k): _serialize_value(v) for k, v in _headers.items()}
        str_path = {'fieldId': _serialize_value(fieldId)}
        # _query always empty
        req = HTTPRequest(method='PUT', url=url, headers=str_headers, path_params=str_path, query_params={}, body=_body)
        resp = await self._client.execute(req)
        return resp
