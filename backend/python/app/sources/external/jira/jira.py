...
Query params:
  - returnDraftIfExists (bool, optional)"""
        ...
        # Only process headers if present and non-empty for conversion
        if headers:
            _headers: Dict[str, str] = _as_str_dict(headers)
        else:
            _headers: Dict[str, str] = {}

        # _path is always {'id': id}, so convert directly and skip function call overhead
        str_id = _serialize_value(id)
        _path: Dict[str, str] = {'id': str_id}

        # Only include query if returnDraftIfExists is not None
        if returnDraftIfExists is not None:
            _query: Dict[str, str] = {'returnDraftIfExists': _serialize_value(returnDraftIfExists)}
        else:
            _query: Dict[str, str] = {}
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # If already correct types, avoid redundant serializations for performance
    # Only called if 'd' is actually non-empty
