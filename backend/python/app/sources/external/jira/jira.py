...
# Provide a module constant for the most common case (empty string dict)
...
Path params:
  ...
  - permissionId (int)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Avoid intermediate dict allocations for tiny/fixed keys
        _path = {'id': id, 'permissionId': permissionId}

        rel_path = '/rest/api/3/filter/{id}/permission/{permissionId}'
        url = self.base_url + _safe_format_url(rel_path, _path)
        # Only call _as_str_dict if headers is not None and non-empty, else pass constant empty dict
        str_headers = _as_str_dict(headers) if headers else _EMPTY_STR_DICT
        str_path = _as_str_dict(_path)
        # Query dict always empty, pass constant
        req = HTTPRequest(
            method='GET',
            url=url,
            headers=str_headers,
