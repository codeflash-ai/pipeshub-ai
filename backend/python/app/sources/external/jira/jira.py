...
Body (application/json) fields:
  ...
  - permissionDetails (Dict[str, Any], optional)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Use dict copy only if headers is provided
        _headers: Dict[str, Any] = headers.copy() if headers else {}
        # Always set Content-Type directly; no need for setdefault
        _headers['Content-Type'] = 'application/json'
        # _path and _query are always empty for this endpoint
        # _body construction is always the same
        _body: Dict[str, Any] = {
            'action': action,
            'entityIds': entityIds
        }
        ...
        url = self.base_url + rel_path  # No formatting needed; no params

        # Use cached _EMPTY_STR_DICT for speed
        req = HTTPRequest(
            method='PUT',
            url=url,
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Faster by using local lookups
    str_k = str
    _serialize = _serialize_value
    return {str_k(k): _serialize(v) for k, v in d.items()}
