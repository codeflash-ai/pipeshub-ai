...
Query params:
  ...
  - returnDraftIfExists (bool, optional)"""
        ...
        # Use empty dict if headers is None for faster branch (avoids dict alloc)
        _headers = headers if headers else {}
        _path = {'id': id}
        
        # Inline query parameter preparation to avoid extra allocation
        if workflowName is not None or returnDraftIfExists is not None:
            _query = {}
            if workflowName is not None:
                _query['workflowName'] = workflowName
            if returnDraftIfExists is not None:
        ...
        # Build request using optimized _as_str_dict and avoid duplicate conversions
        req = HTTPRequest(
            method='GET',
            url=url,
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Fast imperative implementation, slightly faster for tight loops and frequent calls
    result: Dict[str, str] = {}
    for k, v in d.items():
