...
Body: application/json (str)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        _headers = dict(headers) if headers is not None else {}
        _headers.setdefault('Content-Type', 'application/json')
        _path = {'dashboardId': dashboardId, 'itemId': itemId, 'propertyKey': propertyKey}
        rel_path = '/rest/api/3/dashboard/{dashboardId}/items/{itemId}/properties/{propertyKey}'
        url = self.base_url + _safe_format_url(rel_path, _path)
        req = HTTPRequest(
            method='PUT',
            ...
            query_params={},  # Explicitly do not create empty dict via _as_str_dict
            body=body
        )
        resp = await self._client.execute(req)
        return resp
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Use a dict comprehension to minimize memory churn
