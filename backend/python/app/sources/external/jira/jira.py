...
Path params:
  - id (str)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        _headers: Dict[str, Any] = dict(headers or {})
        # avoid dict construction overhead if not necessary on single-key and empty dicts
        _path = {'id': id}
        _query = {}
        _body = None
        rel_path = '/rest/api/3/component/{id}'
        # optimize url formatting for single-key cases
        url = self.base_url + _optimized_format_url(rel_path, _path)
        # fast path as_str_dict for one-entry and empty dicts
        req = HTTPRequest(
            method='GET',
            url=url,
...
def _optimized_format_url(template: str, params: Dict[str, object]) -> str:
    # Performance: Direct single-key string replace for trivial cases, else fallback
    if len(params) == 1:
        k, v = next(iter(params.items()))
        if "{" + k + "}" in template:
            return template.replace("{" + k + "}", str(v))
    try:
        return template.format_map(_SafeDict(params))
    except Exception:
        return template
...
def _fast_as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Performance: Empty and single-item dict fast path for high-frequency calls
    if not d:
        return {}
    if len(d) == 1:
