...
_EMPTY_STR_DICT: Dict[str, str] = {}  # module-level singleton for empty dict
...
HTTP GET /rest/api/3/issuetype"""
        ...
        # Both _path and _query remain empty ({}), so use singleton and avoid repeated conversions
...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    # For empty param dict, skip formatting
    if not params:
        return template
    try:
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    if not d:
        return _EMPTY_STR_DICT  # Return shared instance for empty dict
