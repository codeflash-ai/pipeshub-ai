...
Body (application/json) fields:
  ...
  - workflowNames (list[str], optional)"""
        ...
        req = HTTPRequest(method='POST', url=url, headers=_as_str_dict(_headers) if _headers else {}, path_params=_as_str_dict(_path) if _path else {}, query_params=_as_str_dict(_query) if _query else {}, body=_body)
        resp = await self._client.execute(req)
        return resp
