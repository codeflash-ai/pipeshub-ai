...
Body (application/json) fields:
  ...
  - additionalProperties allowed (pass via body_additional)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Merge/prepare headers efficiently and just once
        _headers: Dict[str, Any] = dict(headers) if headers else {}
        _headers.setdefault('Content-Type', 'application/json')
        _path = {'schemeId': schemeId, 'levelId': levelId}
        _query = {}
        # Build body only once, avoiding repeated key checks/locals usage
        _body = {}
        if description is not None:
            _body['description'] = description
        if name is not None:
        ...
        rel_path = '/rest/api/3/issuesecurityschemes/{schemeId}/level/{levelId}'
        url = self.base_url + _safe_format_url(rel_path, _path)
        # Inline and avoid extra dicts where possible
        req = HTTPRequest(
            method='PUT',
            url=url,
