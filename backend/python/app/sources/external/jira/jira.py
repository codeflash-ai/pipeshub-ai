...
Query params:
  ...
  - maxResult (int, optional)"""
        ...
        if screenId is not None:
            _query['screenId'] = _serialize_value(screenId)
        if tabId is not None:
            _query['tabId'] = _serialize_value(tabId)
        if startAt is not None:
            _query['startAt'] = _serialize_value(startAt)
        if maxResult is not None:
            _query['maxResult'] = _serialize_value(maxResult)
        _body = None
        rel_path = '/rest/api/3/screens/tabs'
        url = self.base_url + _safe_format_url(rel_path, _path)
        req = HTTPRequest(method='GET', url=url, headers=_as_str_dict(_headers), path_params=_as_str_dict(_path), query_params=_query, body=_body)
        resp = await self._client.execute(req)
        return resp
