...
Path params:
  - entityId (str)"""
        ...
        if headers:
            ...
            _headers = {}
        _path = {'entityId': str(entityId)}
        _query: Dict[str, Any] = {}
        _body = None
        rel_path = '/rest/api/3/workflow/{entityId}'
        url = self.base_url + rel_path.replace('{entityId}', str(entityId))
        req = HTTPRequest(method='DELETE', url=url, headers=_headers, path_params=_as_str_dict(_path), query_params=_as_str_dict(_query), body=_body)
        resp = await self._client.execute(req)
        return resp
