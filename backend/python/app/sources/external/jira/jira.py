...
Path params:
  - projectIdOrKey (str)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        return await self._build_request_and_execute(projectIdOrKey, headers)
...
Body: application/json (str)"""
    ...
    async def _build_request_and_execute(self, projectIdOrKey: str, headers: Optional[Dict[str, Any]]=None) -> HTTPResponse:
        _headers = dict(headers) if headers else {}
        _path = {'projectIdOrKey': projectIdOrKey}
        url = self.base_url + _safe_format_url('/rest/api/3/project/{projectIdOrKey}/statuses', _path)
        req = HTTPRequest(
            method='GET',
            url=url,
