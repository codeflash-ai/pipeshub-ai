...
_REL_PATH = '/rest/api/3/issue/{issueIdOrKey}/properties'
...
Path params:
  - issueIdOrKey (str)"""
        ...
        # Avoid new dict alloc unless necessary
        _headers = headers if headers is not None else _EMPTY_DICT

        # Path is always a single key, avoid extra logic
        _path = {'issueIdOrKey': issueIdOrKey}

        # Use the singleton empty dict, will be picked up by optimized _as_str_dict
        _query = _EMPTY_DICT
        _body = None
        ...
        req = HTTPRequest(
            ...
            headers=_as_str_dict(_headers),
            path_params=_as_str_dict(_path),
            query_params=_EMPTY_STR_DICT,  # _as_str_dict(_query) is always _EMPTY_STR_DICT
            body=_body,
        )
        resp = await self._client.execute(req)
        return resp
...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    try:
        ...
        try:
            # Fallback for cases with missing keys, use old logic
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Fast-path for empty dict or already correct mapping.
    if not d:
        return _EMPTY_STR_DICT
    # Check if d is already a dict[str, str]
    if all(isinstance(k, str) and isinstance(v, str) for k, v in d.items()):
        return d  # type: ignore
    # Inline simple _serialize_value for maximum speed
    def _serialize_value(v: Any) -> str:
        if v is None:
            return ''
        if isinstance(v, (list, tuple, set)):
            # Support booleans/str/numbers in iterables
            return ','.join(('true' if x is True else 'false' if x is False else str(x)) for x in v)
