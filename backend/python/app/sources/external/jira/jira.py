...
# Shared immutable empty dict/str-dict objects to avoid repeated allocation
...
HTTP GET /rest/api/3/configuration"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Use shared empty dict to avoid alloc if headers is None or empty
        _headers: Dict[str, Any] = headers if headers else _EMPTY_DICT
        rel_path = '/rest/api/3/configuration'
        # _path and _query always empty so can use shared constants below
...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    # Fast path: if params is empty, skip formatting
    if not params:
        return template
    try:
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Fast path: avoid dict comprehension on empty
    if not d:
