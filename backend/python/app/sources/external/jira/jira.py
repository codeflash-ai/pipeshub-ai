...
# Cache static empty str dict to avoid unnecessary recreations
...
Body (application/json) fields:
  ...
  - sendBulkNotification (bool, optional)"""
        ...
        _headers.setdefault('Content-Type', 'application/json')
        # _path and _query are always empty; avoid allocation, will be passed as empty dict
        ...
        # rel_path is static, has no substitutions: skip formatting for performance
        url = self.base_url + '/rest/api/3/bulk/issues/delete'
        req = HTTPRequest(
            method='POST',
            url=url,
