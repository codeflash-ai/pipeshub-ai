...
Query params:
  ...
  - expand (str, optional)"""
        ...
        # Preserve copy semantics in building _headers
        _headers: Dict[str, Any] = dict(headers) if headers else {}

        # Construct _query inline, skipping empty initializations
        _query: Dict[str, Any] = {}
        if groupname is not None:
            _query['groupname'] = groupname
        if groupId is not None:
        ...
        # Avoid redundant _as_str_dict if dict is empty
        req = HTTPRequest(
            method='GET',
            url=url,
...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    # If params is empty, skip extra instantiation and formatting
    if not params:
        return template
    try:
