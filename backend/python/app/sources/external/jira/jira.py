...
Path params:
  - id (int)"""
        ...
        if headers:
            ...
            headers_str = {}
        _path: Dict[str, Any] = {'id': id}
        path_str = {'id': _serialize_value(id)}
        _query: Dict[str, Any] = {}
        _body = None
        rel_path = '/rest/api/3/filter/{id}'
        url = self.base_url + _safe_format_url(rel_path, _path)
        req = HTTPRequest(method='DELETE', url=url, headers=headers_str, path_params=path_str, query_params={}, body=_body)
        resp = await self._client.execute(req)
        return resp
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Small dict optimization for empty or singleton dict
    if not d:
        return {}
    if len(d) == 1:
