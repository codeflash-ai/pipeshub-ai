...
Query params:
  ...
  - swapGroupId (str, optional)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Avoid unnecessary allocation: only copy headers if not None
        _headers: Dict[str, Any] = headers if headers is not None else {}
        # Build _query dict directly, avoids unnecessary empty dicts
        _query: Dict[str, Any] = {}
        if groupname is not None:
            _query['groupname'] = groupname
        if groupId is not None:
        ...
        req = HTTPRequest(
            ...
            url=url,
            headers=_as_str_dict(_headers),
            path_params={},  # path params unused, pass empty dict
            query_params=_as_str_dict(_query),
            body=_body,
        )
        resp = await self._client.execute(req)
