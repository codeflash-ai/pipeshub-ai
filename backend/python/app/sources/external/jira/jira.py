...
# Static empty string dict for fast re-use
...
Query params:
  ...
  - fieldConfigurationSchemeId (list[int], optional)"""
        ...
        req = HTTPRequest(
            method='GET',
            url=url,
            headers=_as_str_dict(_headers) if _headers else EMPTY_STR_DICT,
            path_params=EMPTY_STR_DICT,
            query_params=_as_str_dict(_query) if _query else EMPTY_STR_DICT,
            body=_body,
        )
        resp = await self._client.execute(req)
        return resp
