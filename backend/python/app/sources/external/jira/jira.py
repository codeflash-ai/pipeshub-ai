...
Query params:
  ...
  - maxResults (int, optional)"""
        ...
        # Fast path: Set up dictionaries efficiently
        if headers:
        ...
        # Avoid converting empty dicts if possible
        req = HTTPRequest(
            method='GET',
            url=url,
...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    # Fast path: Only attempt format if params are not empty
    if params:
        try:
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Optimized for lower-overhead conversion for small dicts
    out = {}
    for k, v in d.items():
