...
Path params:
  - issueIdOrKey (str)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Only perform conversion for headers if needed (possible non-str values), else use directly
        if headers:
            _headers: Dict[str, Any] = dict(headers)
        else:
            _headers: Dict[str, Any] = {}

        # _path: keys/values always str, so skip conversion
        _path = {'issueIdOrKey': issueIdOrKey}
        # _query: always empty, so pass empty dict
        _query = {}

        _body = None
        rel_path = '/rest/api/3/issue/{issueIdOrKey}/watchers'
        # For performance, use plain dict when values are str
        url = self.base_url + rel_path.format_map(_path)
        req = HTTPRequest(
            method='GET',
            url=url,
            headers=_as_str_dict(_headers),
            path_params=_path,   # Use raw dict as keys/values are already str
            query_params=_query, # Use raw empty dict instead of conversion
            body=_body,
        )
        resp = await self._client.execute(req)
        return resp
