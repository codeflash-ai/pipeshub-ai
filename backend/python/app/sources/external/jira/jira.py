...
Path params:
  - type (str)"""
        ...
        # _query is always empty here, so don't compute it every call
        str_query = {}
...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    # Minor: shortcut for empty params
    if not params:
        return template
    try:
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    """Convert dict of any to dict[str, str]; fast path for empty or all-str dicts."""
    if not d:
        return {}
    # Fast path: all keys and values are already str
    if all(isinstance(k, str) and isinstance(v, str) for k, v in d.items()):
        return d  # Safe since external code will not mutate the returned value
