...
Body (application/json) fields:
  ...
  - workflows (list[Dict[str, Any]], optional)"""
        ...
        # Fast dict preparation for headers
        if headers:
        ...
        # Avoid unnecessary conversion: path/query are always empty here
        rel_path = '/rest/api/3/workflows/update'
        url = f"{self.base_url}{rel_path}"

        # Prepare body as needed
        _body: Dict[str, Any] = {}
        if statuses is not None:
            _body['statuses'] = statuses
        if workflows is not None:
        ...
        # Only convert headers to str for request (path/query always empty)
        req = HTTPRequest(
            method='POST',
            url=url,
