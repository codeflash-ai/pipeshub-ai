...
Body (application/json) fields:
  ...
  - workflowSchemeId (str, optional)"""
        ...
        # Efficient header creation & merging
        if headers:
        ...
        # Only populate _body with needed keys via literal
        _body: Dict[str, Any] = {'projectId': projectId}
        if workflowSchemeId is not None:
            _body['workflowSchemeId'] = workflowSchemeId
        rel_path = '/rest/api/3/workflowscheme/project'
        # If there are no path/query params, skip conversion for performance
        url = self.base_url + _safe_format_url(rel_path, {})
        req = HTTPRequest(
            method='PUT',
            url=url,
            headers=_as_str_dict(_headers),
            path_params={},         # No need to convert empty dict
            query_params={},        # No need to convert empty dict
            body=_body
        )
        resp = await self._client.execute(req)
        return resp
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # If the dict is empty, just return {} instantly
    if not d:
        return {}
