...
Query params:
  - redirect (bool, optional)"""
        ...
        # Convert headers and query in-place only if needed; avoids allocating intermediate dicts
        req = HTTPRequest(
            method='GET',
            url=url,
            headers=_as_str_dict(headers) if headers else {},
            path_params={'id': str(id)},
            query_params={'redirect': _serialize_value(redirect)} if redirect is not None else {},
            body=None,
        )
        resp = await self._client.execute(req)
        return resp
