...
Query params:
  - accountId (str, required)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Optimize: Only convert headers if provided and non-empty
        if headers:
            _headers: Dict[str, str] = _as_str_dict(headers)
        else:
            _headers = {}
        # _path is always empty; optimization: skip conversion
        _path: Dict[str, str] = {}
        # Optimize: construct _query_params directly, since it's always just accountId
        _query_params: Dict[str, str] = {'accountId': _serialize_value(accountId)}
        _body = None
        # Fast path: rel_path does not require formatting, so skip _safe_format_url
        url = self.base_url + '/rest/api/3/user/email'
        req = HTTPRequest(
            method='GET',
            url=url,
            headers=_headers,
