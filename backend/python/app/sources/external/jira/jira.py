...
HTTP GET /rest/api/3/filter/defaultShareScope"""
        ...
        # Avoid unnecessary dict copy when headers is empty or None
        _headers = headers if headers else {}

        rel_path = '/rest/api/3/filter/defaultShareScope'
        url = self.base_url + _safe_format_url(rel_path, {})

        # Inlining assignments, passing empty dicts directly
        req = HTTPRequest(
            method='GET',
            url=url,
            headers=_as_str_dict(_headers),
            path_params={},  # avoid call to _as_str_dict since it's always empty
            query_params={}, # avoid call to _as_str_dict since it's always empty
            body=None,
        )
        resp = await self._client.execute(req)
        return resp
...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    # If params is empty, avoid constructing _SafeDict and formatting.
    if not params:
        return template
    try:
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Avoid conversion for empty dicts, hot path optimization
    if not d:
        return {}
