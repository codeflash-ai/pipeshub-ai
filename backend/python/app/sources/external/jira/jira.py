...
Body (application/json) fields:
  ...
  - projectId (str, required)"""
        client = self._client  # reduce attribute lookup
        if client is None:
            raise ValueError('HTTP client is not initialized')

        # _headers construction
        if headers:
        ...
        # _path and _query are always empty
        _path = {}
        _query = {}

        # Prepare _body efficiently
        _body = {'projectId': projectId}
        if fieldConfigurationSchemeId is not None:
            _body['fieldConfigurationSchemeId'] = fieldConfigurationSchemeId
        rel_path = '/rest/api/3/fieldconfigurationscheme/project'

        # Fast path: when _path is empty, skip formatting
        url = self.base_url + (rel_path if not _path else _safe_format_url(rel_path, _path))
        ...
        req = HTTPRequest(
            ...
            url=url,
            headers=headers_str,
            path_params={},  # _path is always empty here; skip conversion
            query_params={}, # _query is always empty here; skip conversion
            body=_body
        )
...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    # Fast path: params empty, skip format_map
    if not params:
        return template
    try:
