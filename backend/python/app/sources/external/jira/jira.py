...
HTTP GET /rest/atlassian-connect/1/app/module/dynamic"""
        ...
        _headers = headers if headers is not None else {}
        # These are always empty per signature; avoid unnecessary conversions
        _path = {}
        _query = {}
        _body = None
        rel_path = '/rest/atlassian-connect/1/app/module/dynamic'
        # Avoid format_map if params are always empty
        url = f'{self.base_url}{rel_path}'
        # Avoid calling _as_str_dict for known, always empty dicts
        req = HTTPRequest(
            method='GET',
            url=url,
