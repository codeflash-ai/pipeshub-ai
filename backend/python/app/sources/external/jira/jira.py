...
Path params:
  - id (str)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Avoid allocation for {} and single key dicts
        _headers = headers if headers else {}
        # Direct trivial dict for path
        _path = {'id': id}
        _query = {}
        _body = None
        rel_path = '/rest/api/3/component/{id}'
        ...
        req = HTTPRequest(
            ...
            url=url,
            headers=_as_str_dict(_headers),
            path_params={'id': str(id)}, # Fast path for single key, less overhead
            query_params=_as_str_dict(_query),
            body=_body,
        )
        resp = await self._client.execute(req)
...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    # Fast-path for common case: single id parameter, template only contains {id}
    if template == '/rest/api/3/component/{id}' and 'id' in params:
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    ...
    if not d:
        return {}
    # Single key, value directly stringifiable (common for path)
    if len(d) == 1:
        (k, v), = d.items()
