...
Path params:
  - linkId (str)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # No need to re-wrap empty dict for headers, use as-is if headers is None.
        _headers: Dict[str, Any] = headers if headers is not None else {}
        # _path has a known constant structure, can optimize.
        # No need for an intermediate dict (as _as_str_dict is optimized below).
        # _query and _body are always empty/None.
        rel_path = '/rest/api/3/issueLink/{linkId}'
        # Fast-path: format the known template without building SafeDict each time.
        url = self.base_url + _fast_format_issue_link(rel_path, linkId)
        # Optimized _as_str_dict for single-key dict (hot path).
        str_headers = _as_str_dict(_headers)
        str_path = {'linkId': str(linkId)}  # serialize_value is unnecessary for simple str
        # _query is always empty
        req = HTTPRequest(
            method='GET',
            url=url,
            headers=str_headers,
            path_params=str_path,
            query_params={},  # _as_str_dict for empty dict is redundant
            body=None
        )
        resp = await self._client.execute(req)
        return resp
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Inline the specialist for empty and single-key dict (since that's common here)
    if not d:
        return {}
    if len(d) == 1:
        k, v = next(iter(d.items()))
        return {str(k): _serialize_value(v)}
    # fallback for rare multi-key or complex dicts
    return {str(k): _serialize_value(v) for (k, v) in d.items()}


def _fast_format_issue_link(template: str, linkId: object) -> str:
    # rel_path is always '/rest/api/3/issueLink/{linkId}'
    # inline formatting for this specific template, avoiding format_map + SafeDict overhead
    # string concatenation is safe here
    # Must ensure linkId is string for insertion
    return template.replace('{linkId}', str(linkId), 1)
