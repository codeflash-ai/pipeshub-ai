...
Query params:
  - replaceWith (str, optional)"""
        ...
        # Headers are typically string keys and values, so avoid serializing unless necessary
        _headers = headers if headers is not None else {}
        # path params are always str in this endpoint by typing, skip as_str_dict
        _path = {'schemeId': schemeId, 'levelId': levelId}
        # Query params, only insert if present
        if replaceWith is not None:
            _query = {'replaceWith': replaceWith}
        else:
            _query = {}
        ...
        # Direct dict-string conversion only for those fields possibly non-string
        req = HTTPRequest(
            method='DELETE',
            url=url,
            headers=_as_str_dict(_headers),
            path_params=_path,  # Already string types
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    ...
    if all(isinstance(k, str) and (isinstance(v, str) or v is None) for k, v in d.items()):
        # Only call _serialize_value if not already a string or None
        # This preserves original behavior for None values
        return {k: _serialize_value(v) if v is not None else '' for k, v in d.items()}
