...
Path params:
  - id (int)"""
        ...
        # Perform string conversion in-place and reuse objects
        _headers: Dict[str, str] = _as_str_dict(headers) if headers else {}
        _path: Dict[str, str] = {'id': _serialize_value(id)}
        _query: Dict[str, str] = {}  # query dict is already empty, no need to convert
        _body = None
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Early exit for None and already-typed dicts (optimization)
    if not d:
        return {}
    # Simple check to avoid unnecessary work for already stringified items
