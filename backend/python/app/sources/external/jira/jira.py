...
Body (application/json) fields:
  ...
  - additionalProperties allowed (pass via body_additional)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Direct dict construction - single allocation
        _headers: Dict[str, Any] = dict(headers) if headers else {}
        _headers.setdefault('Content-Type', 'application/json')
        _path: Dict[str, Any] = {'fieldKey': fieldKey}
        _query: Dict[str, Any] = {}
        # Body dict built in-place for lowest alloc overhead
        _body: Dict[str, Any] = {'value': value}
        if config is not None:
            _body['config'] = config
        if properties is not None:
            _body['properties'] = properties
        # Avoid using 'locals()' pattern, fast conditional
        if body_additional:
...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    # Ultra-fast path if no placeholders
    if '{' not in template:
        return template
    try:
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    ...
    if not d:
        return {}
    # Use local variable for the function (if imported, e.g.)
    # _serialize_value = ... # Would cache if imported
