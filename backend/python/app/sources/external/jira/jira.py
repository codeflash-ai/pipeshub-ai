...
# Constants (for minor allocation efficiency and clarity)
_EMPTY_DICT: Dict[str, Any] = {}
_EMPTY_STR_DICT: Dict[str, str] = {}
_REL_PATH: str = '/rest/api/3/fieldconfigurationscheme/project'
...
Query params:
  ...
  - projectId (list[int], required)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Avoids unnecessary copy/allocation if headers is None or already dict
        if headers:
            _headers: Dict[str, Any] = dict(headers)
        else:
            _headers = {}
        # These are always empty for this endpoint; use static singleton for efficiency
        _path = _EMPTY_DICT
        _query = {}
        if startAt is not None:
            _query['startAt'] = startAt
        ...
        _query['projectId'] = projectId
        _body = None
        # Precompute string constant for rel_path
        rel_path = _REL_PATH
        # For this endpoint _path is always {}, so _safe_format_url will just return rel_path directly
        url = self.base_url + _safe_format_url(rel_path, _path)
        req = HTTPRequest(
            method='GET',
            url=url,
...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    # Fast path for empty params, which is the common case for this API
    if not params:
        return template
    try:
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    ...
    # Avoid calling str() unnecessarily for keys that are already strings and use direct assignment for string values
