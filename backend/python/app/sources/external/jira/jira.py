...
_REL_PATH_VOTES = '/rest/api/3/issue/{issueIdOrKey}/votes'
...
Path params:
  - issueIdOrKey (str)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Avoid dict allocation: only copy if headers are provided
        if headers:
            _headers = dict(headers)
            headers_str = _as_str_dict(_headers)
        else:
            headers_str = _EMPTY_STR_DICT

        # Only one path param; more efficient without dict comprehension
        path = {'issueIdOrKey': issueIdOrKey}
        path_str = {'issueIdOrKey': _serialize_value(issueIdOrKey)}

        # Query and body are always empty/static
        query_str = _EMPTY_STR_DICT
        _body = None

        url = self.base_url + _safe_format_url(_REL_PATH_VOTES, path)
        req = HTTPRequest(
            method='GET',
            url=url,
            headers=headers_str,
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Avoids unnecessary dict allocation; only convert if key/value not already string
    if not d:
