...
Path params:
  - id (int)"""
        ...
        # Fast-path for headers (empty or None)
        if headers:
        ...
        _query: Dict[str, Any] = {}
        id_str = str(id)
        _path: Dict[str, Any] = {'id': id_str}
        _body = None
        rel_path = '/rest/api/3/fieldconfigurationscheme/{id}'
        url = self.base_url + _safe_format_url(rel_path, _path)

        # Only call _as_str_dict on non-empty dicts (avoid unnecessary work)
        headers_str = _as_str_dict(_headers) if _headers else {}
        # _path always has single key ('id'), id_str is always str
        path_params_str = _path  # already converted above
        # _query is always empty dict here
        query_params_str = {}
...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    if not params:
        return template
    # Slightly faster: avoid exception catching where not needed
    try:
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    ...
    if not d:
        return {}
    # Optimize single-item or all-str case
    out = {}
    for k, v in d.items():
        keystr = k if isinstance(k, str) else str(k)
        valstr = _serialize_value(v) if not (isinstance(v, str)) else v
        out[keystr] = valstr
    return out
