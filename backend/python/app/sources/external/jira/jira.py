...
Path params:
  ...
  - id (str)"""
        ...
        # Optimize header preparation (avoid dict() if headers None)
        if headers:
            _headers: Dict[str, Any] = {**headers}
        else:
            _headers: Dict[str, Any] = {}

        # These path keys are always str, _serialize_value optimizes for None
        _path: Dict[str, Any] = {'issueIdOrKey': issueIdOrKey, 'id': id}
        rel_path = '/rest/api/3/issue/{issueIdOrKey}/comment/{id}'
        url = self.base_url + _safe_format_url(rel_path, _path)

        # Only convert dicts if non-empty
        as_str_headers = _as_str_dict(_headers) if _headers else {}
        as_str_path = _as_str_dict(_path)
        # Since _query is always empty, pass {} directly without conversion
        req = HTTPRequest(
            method='DELETE',
            url=url,
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Optimize: avoid repeated str() for str keys
