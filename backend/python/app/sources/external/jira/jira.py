...
Path params:
  ...
  - propertyKey (str)"""
        ...
        url = self.base_url + _safe_format_url(rel_path, _path)
        _headers = headers if headers is not None else {}
        # Convert all dicts once and only when necessary
        req = HTTPRequest(
            method='DELETE',
            url=url,
            headers=_as_str_dict(_headers) if _headers else {},
            path_params={'addonKey': str(addonKey), 'propertyKey': str(propertyKey)},
            query_params={},
            body=None
        )
        resp = await self._client.execute(req)
...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    try:
        # This assumes all keys in params are present in the template
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Inline import to avoid circular import and only use when needed
