...
Path params:
  ...
  - propertyKey (str)"""
        ...
        rel_path = '/rest/api/3/comment/{commentId}/properties/{propertyKey}'
        url = f'{base_url}/rest/api/3/comment/{commentId}/properties/{propertyKey}'

        # Inline the header and path param serialization for performance; skip empty dict serial if possible
        _headers = headers if headers else {}
        if _headers:
            # Use only those conversions that are needed (as >99% are string->string)
            # Only convert non-str keys/vals
            str_headers = {}
            for k, v in _headers.items():
                if type(k) is str and type(v) is str:
                    str_headers[k] = v
                else:
                    str_headers[str(k)] = _serialize_value(v)
        else:
            str_headers = {}

        path_params = {'commentId': commentId, 'propertyKey': propertyKey}
