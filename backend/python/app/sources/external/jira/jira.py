...
Body (application/json) fields:
  - updates (list[Dict[str, Any]], optional)"""
        ...
        _query = {'generateChangelog': generateChangelog} if generateChangelog is not None else {}
        _body = {'updates': updates} if updates is not None else {}

        rel_path = '/rest/api/3/app/field/{fieldIdOrKey}/value'
        url = self.base_url + _safe_format_url(rel_path, _path)
        # Avoid unnecessary repeated work in as_str_dict for empty cases
        req = HTTPRequest(
            method='PUT',
            url=url,
...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    try:
        # Using direct replacement instead of format_map for speed
        value = str(params['fieldIdOrKey'])
        return template.replace('{fieldIdOrKey}', value)
    except Exception:
        return template
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Fast-path empty dicts
    if not d:
        return {}
    # Avoid creating a new dict and repeated str() if not needed
    items = d.items()
