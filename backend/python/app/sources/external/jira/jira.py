...
Path params:
  - idOrName (str)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Optimize headers - avoid dict() allocation when None
        _headers: Dict[str, Any] = headers.copy() if headers is not None else {}
        # Avoid as_str_dict call for single-path param
        _path_str: Dict[str, str] = {'idOrName': _serialize_value(idOrName)}
        # Skip query param serialization for empty dict
        _query_str: Dict[str, str] = {}
        _body = None
        rel_path = '/rest/api/3/status/{idOrName}'
        # Optimize URL formatting for single variable
        url = (
            self.base_url + rel_path.replace('{idOrName}', _path_str['idOrName'])
            if 'idOrName' in _path_str and '{idOrName}' in rel_path
            else self.base_url + _safe_format_url(rel_path, {'idOrName': idOrName})
        )
        # Only call _as_str_dict once for headers
        req = HTTPRequest(
            method='GET',
            url=url,
...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    # Optimization: For single variable paths, use string replace directly
    if len(params) == 1:
        (k, v), = params.items()
        placeholder = '{' + k + '}'
        if placeholder in template:
            return template.replace(placeholder, _serialize_value(v))
    try:
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Fast-path for empty and single-element dicts
    if not d:
        return {}
    if len(d) == 1:
