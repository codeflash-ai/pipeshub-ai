...
_BULK_FETCH_REL_PATH = '/rest/api/3/issue/bulkfetch'
...
Body (application/json) fields:
  ...
  - properties (list[str], optional)"""
        ...
        # Avoid copying dict if headers is None or empty
        if headers:
        ...
        # Fill body with only present arguments, avoids wasted assignments
        _body: Dict[str, Any] = {'issueIdsOrKeys': issueIdsOrKeys}
        if expand is not None:
            _body['expand'] = expand
        if fields is not None:
            _body['fields'] = fields
        ...
        rel_path = _BULK_FETCH_REL_PATH
        # Optimize: Only call _safe_format_url if _path is not empty
        url = self.base_url + (_safe_format_url(rel_path, _path) if _path else rel_path)

        # optimize: only call _as_str_dict on non-empty dicts; for empty dicts, pass {} directly (no need for comprehension)
        req = HTTPRequest(
            method='POST',
            url=url,
...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    # Optimization: Fast return if params is empty (most common/hot path)
    if not params:
        return template
    try:
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Optimization: Check empty before comprehension for hot path
    if not d:
        return {}
