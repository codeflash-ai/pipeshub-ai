...
Body (application/json) fields:
  ...
  - additionalProperties allowed (pass via body_additional)"""
        ...
        # Build headers in-place, avoid unnecessary copies
        _headers: Dict[str, Any] = dict(headers) if headers else {}
        if 'Content-Type' not in _headers:
            _headers['Content-Type'] = 'application/json'

        # Build query params using efficient dict comprehension
        _query: Dict[str, Any] = {}
        if groupname is not None:
            _query['groupname'] = groupname
        if groupId is not None:
        ...
        # Build body in a single dict, including optional additional fields
        _body = {}
        if accountId is not None:
            _body['accountId'] = accountId
        if name is not None:
        ...
        if body_additional:
            _body.update(body_additional)
        rel_path = '/rest/api/3/group/user'
        url = self.base_url + _safe_format_url(rel_path, {})  # _path is always empty here

        # Convert dicts to string-dict only once, avoid repeated conversions
        req = HTTPRequest(
            method='POST',
            url=url,
            headers=_as_str_dict(_headers),
            path_params={},  # no path params (dict always empty)
            query_params=_as_str_dict(_query),
            body=_body,
        )
        resp = await self._client.execute(req)
