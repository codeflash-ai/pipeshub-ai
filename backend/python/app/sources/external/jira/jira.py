...
Query params:
  ...
  - gadgetId (list[int], optional)"""
        ...
        # Fast-path: only populate fields if they exist, minimize allocations
        _path_dashboardId = dashboardId
        # Since only one path param, avoid dict allocation
        rel_path = '/rest/api/3/dashboard/{dashboardId}/gadget'
        url = self.base_url + rel_path.replace('{dashboardId}', str(_path_dashboardId))

        # Query and headers dicts creation minimized
        _headers = headers if headers is not None else {}
        # If no headers, avoid dict() allocation; if headers are already dict, use as-is
        _headers_str = _as_str_dict(_headers) if _headers else {}

        # Only add keys to _query if not None, minimizing unnecessary empty dict construction
        _query = {}
        ...
        _path_str = {'dashboardId': str(_path_dashboardId)}

        req = HTTPRequest(
            method='GET',
            url=url,
            headers=_headers_str,
            path_params=_path_str,  # Already string, no need for conversion
