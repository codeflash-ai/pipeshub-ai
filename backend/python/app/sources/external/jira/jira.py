...
Path params:
  ...
  - id (str)"""
        ...
        # Using fast copy if headers not None, else avoid unnecessary allocation
        _headers: Dict[str, Any] = headers.copy() if headers else {}

        # Path can safely be created inline; no change needed
        _path: Dict[str, Any] = {
            'issueIdOrKey': issueIdOrKey,
            'id': id,
        }
        ...
        # Only convert if necessary; micro-optimize for empty/small dicts
        hdr = _as_str_dict(_headers) if _headers else {}
        pth = _as_str_dict(_path)
        qry = _as_str_dict(_query) if _query else {}
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Fast exit for common case (empty)
    if not d:
        return {}
    # No type-strictness guarantees, so convert keys/values only if not already strings
    # This avoids unnecessary conversions if all values are str
    out = {}
    for k, v in d.items():
        ...
        if type(v) is str:
            ...
            vs = _serialize_value(v)
        out[ks] = vs
    return out

def _to_bool_str(v: Any) -> str:
    # Fast-path for bool conversion, else str()
    if type(v) is bool:
        return 'true' if v else 'false'
    return str(v)
