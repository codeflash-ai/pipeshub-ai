...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    # Fast-path: if params is empty, skip formatting and return template directly
    if not params:
        return template
    try:
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Fast-path: empty dict results in empty output with no iteration/str/serialization
    if not d:
        return {}
