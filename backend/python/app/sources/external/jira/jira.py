...
class JiraDataSource:
    def __init__(self, client: JiraClient) -> None:
        ...
        try:
            # Only rstrip if needed, avoid unnecessary string copy
            base_url = self._client.get_base_url()
            self.base_url = base_url if not base_url.endswith('/') else base_url.rstrip('/')
...
Body (application/json) fields:
  ...
  - additionalProperties allowed (pass via body_additional)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Use headers only if provided, minimize dict construction
        _headers: Dict[str, Any] = dict(headers) if headers else {}
        _headers.setdefault('Content-Type', 'application/json')
        _path = {'issueIdOrKey': issueIdOrKey}
        _query = {}
        # _body dict built eagerly, minimize ops
        _body = {}
