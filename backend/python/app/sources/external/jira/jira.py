...
Body (application/json) fields:
  ...
  - description (str, optional)
  - name (str, optional)"""
        # Cache _client/state outside hot path
        client = self._client
        if client is None:
            raise ValueError('HTTP client is not initialized')

        # Make header dict with minimal intermediate allocation
        headers_dict = dict(headers) if headers else {}
        headers_dict.setdefault('Content-Type', 'application/json')

        path_dict = {'issueTypeSchemeId': issueTypeSchemeId}

        # Build body using a single pass
        body_dict = {}
        if defaultIssueTypeId is not None:
            body_dict['defaultIssueTypeId'] = defaultIssueTypeId
        if description is not None:
        ...
        rel_path = '/rest/api/3/issuetypescheme/{issueTypeSchemeId}'
        url = self.base_url + _safe_format_url(rel_path, path_dict)

        req = HTTPRequest(
            method='PUT',
            url=url,
            headers=_as_str_dict(headers_dict),
            path_params=_as_str_dict(path_dict),
            query_params={},  # Query is always empty here
            body=body_dict,
        )
