...
# Use an immutable, shared empty dict for efficiency
...
Path params:
  ...
  - tabId (int)"""
        ...
        # Fast path: empty/default header
        if headers:
            _headers: Dict[str, Any] = dict(headers)
            headers_dict = _as_str_dict(_headers)
        else:
            _headers = {}
            headers_dict = EMPTY_DICT

        # Fixed path, fixed known param names, direct formatting for performance
        _path: Dict[str, Any] = {'screenId': screenId, 'tabId': tabId}
        path_dict = _as_str_dict(_path)

        # The path always exists (never empty), so no need to check shortcut
        rel_path = '/rest/api/3/screens/{screenId}/tabs/{tabId}'
        # Since rel_path and _path keys are always known and mandatory, f-string direct format is safe here and faster
        url = f"{self.base_url}/rest/api/3/screens/{screenId}/tabs/{tabId}"

        _query: Dict[str, Any] = {}
        # Fast path for empty query
        query_dict = EMPTY_DICT
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Fast path for empty dicts: most efficient way is to return shared EMPTY_DICT,
    # otherwise fallback to comprehension for real cases
    if not d:
        return EMPTY_DICT
