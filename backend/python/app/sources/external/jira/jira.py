...
Body (application/json) fields:
  ...
  - nextPageToken (str, optional)"""
        ...
        # Fast-path the very common case: path dict always empty, so avoid format_map and exception handling
        if not _path:
            url = f"{self.base_url}{rel_path}"
            # These conversions are always to empty-str dicts, so reuse
            str_path = _EMPTY_STR_DICT
        else:
            url = self.base_url + _safe_format_url(rel_path, _path)
            str_path = _as_str_dict(_path)

        str_headers = _as_str_dict(_headers) if _headers else _EMPTY_STR_DICT
        str_query = _as_str_dict(_query) if _query else _EMPTY_STR_DICT
