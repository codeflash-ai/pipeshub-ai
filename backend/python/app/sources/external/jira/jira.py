...
Path params:
  ...
  - propertyKey (str)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        _headers = headers if headers and all(isinstance(k,str) and isinstance(v,str) for k,v in headers.items()) else _as_str_dict(headers or {})
...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    try:
        ...
        # Fall back to safe path (should not be needed in practice)
        try:
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Fast path: empty dict, or already str->str
    if not d:
        return {}
