...
HTTP GET /rest/api/3/priority"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        _headers: Dict[str, Any] = dict(headers) if headers else _EMPTY_STR_DICT
        _path: Dict[str, Any] = {}
        _query: Dict[str, Any] = {}
        _body = None
        rel_path = '/rest/api/3/priority'
        # Fast path for empty _path dict
        url = self.base_url + rel_path if not _path else self.base_url + _safe_format_url(rel_path, _path)
        # Precompute string dicts only if dicts are non-empty
        headers_str = _EMPTY_STR_DICT if not _headers else _as_str_dict(_headers)
        path_str = _EMPTY_STR_DICT if not _path else _as_str_dict(_path)
        query_str = _EMPTY_STR_DICT if not _query else _as_str_dict(_query)
        req = HTTPRequest(
            method='GET',
            url=url,
            headers=headers_str,
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Fast path for empty dicts or identity
    if not d:
