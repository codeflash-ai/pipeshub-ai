...
Path params:
  - issueTypeId (str)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Fast, direct copy if headers is not None, otherwise empty dict.
        if headers is None:
            _headers: Dict[str, Any] = {}
        elif type(headers) is dict:
            _headers = headers.copy()
        else:  # Defensive: handle for mappings that are not dict
            _headers = dict(headers)
        _path: Dict[str, Any] = {'issueTypeId': issueTypeId}
        rel_path = '/rest/api/3/issuetype/{issueTypeId}/properties'
        url = self.base_url + _safe_format_url(rel_path, _path)
        # Only create dicts and call _as_str_dict if nonempty for memory/cpu efficiency
        req = HTTPRequest(
            method='GET',
            url=url,
...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    # Optimize for the specific template used
    issue_type_id = params.get('issueTypeId')
    if issue_type_id is not None:
        return template.replace('{issueTypeId}', str(issue_type_id))
    try:
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Short-circuit if input dict is empty
    if not d:
        return {}
