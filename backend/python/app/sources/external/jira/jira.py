...
Query params:
  ...
  - maxResults (int, optional)"""
        ...
        _query = None
        if nextPageToken is not None or maxResults is not None:
            # Reuse dict only if actually needed
            _query = {}
            if nextPageToken is not None:
                _query['nextPageToken'] = nextPageToken
            if maxResults is not None:
        ...
        # Use direct string formatting for single path parameter (statusId), avoids SafeDict and .format_map
        rel_path = '/rest/api/3/statuses/{statusId}/workflowUsages'
        url = f"{self.base_url}/rest/api/3/statuses/{_serialize_status_id(statusId)}/workflowUsages"

        req = HTTPRequest(
            method='GET',
            url=url,
            headers=_as_str_dict(_headers) if _headers else {},
            path_params={'statusId': statusId},
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Optimize: direct return for empty or None dict, avoids unnecessary comprehension
    if not d:
        return {}
    return {str(k): _serialize_value(v) for (k, v) in d.items()}


def _serialize_status_id(statusId: str) -> str:
    # Fast path for direct safe conversion, if any escaping needed could be performed here.
    return str(statusId)
