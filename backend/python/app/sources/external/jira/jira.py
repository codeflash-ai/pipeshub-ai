...
# Reuse empty dict to avoid repeated allocations
...
Query params:
  ...
  - orderBy (str, optional)"""
        ...
        url = self.base_url + _safe_format_url(rel_path, _path)
        req = HTTPRequest(method='GET', url=url, headers=_as_str_dict(_headers) if _headers else _EMPTY_STR_DICT, path_params=_EMPTY_STR_DICT, query_params=_as_str_dict(_query) if _query else _EMPTY_STR_DICT, body=_body)
        resp = await self._client.execute(req)
        return resp
...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    # Fast-path: Avoid formatting if no params. On error, also fallback.
    if not params:
        return template
    try:
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    if not d:
        return {}
    # The _serialize_value function is defined elsewhere (read-only)
