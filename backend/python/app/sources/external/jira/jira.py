...
class _SingleKeyDict(dict):
    # Minimal SafeDict for single-key usage to avoid unnecessary copy or logic
...
Path params:
  - applicationKey (str)"""
        ...
        # Direct allocation of headers and params, avoiding unnecessary dict wrapping/conversion
        _headers = headers if headers is not None else {}
        _path = {'applicationKey': applicationKey}
        _query = {}
        _body = None
        rel_path = '/rest/api/3/license/approximateLicenseCount/product/{applicationKey}'
        # Avoid _SafeDict for the simple one-key path, catch KeyError if any
        try:
            url = self.base_url + rel_path.format_map(_SingleKeyDict(_path))
        except Exception:
            url = self.base_url + rel_path

        # Inline str conversion and serialization for dicts using fast list comprehension
        req = HTTPRequest(
            method='GET',
            url=url,
...
def _to_bool_str(v: Any) -> str:
    # Avoid import overhead and use str for all except bool
    if isinstance(v, bool):
        return 'true' if v else 'false'
    return str(v)

def _fast_as_str_dict(d: Dict[Any, Any]) -> Dict[str, str]:
    # Use direct key/value conversion, minimize overhead, skip conversion for identity
    if not d:
        return {}
    # Small dicts path: avoid generator
