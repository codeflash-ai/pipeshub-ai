...
Query params:
  ...
  - projectId (list[int], required)"""
        ...
        # These are always empty or single assignment, faster as literal
        _path = {}
        # Only assign needed keys, avoid unnecessary dict construction
        _query = {}
        if startAt is not None:
            _query['startAt'] = startAt
        if maxResults is not None:
...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    # Fast path for empty params - avoids allocating SafeDict or format_map
    if not params:
        return template
    try:
...
def _to_bool_str(v: Any) -> str:
    # This mirrors safe string conversion for basic values
    if isinstance(v, bool):
        return 'true' if v else 'false'
    return str(v)
