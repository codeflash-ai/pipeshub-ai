...
Query params:
  ...
  - maxResults (int, optional)"""
        ...
        # Fast path: since _path is always empty, skip formatting call
        url = self.base_url + _REL_PATH

        req = HTTPRequest(
            method='GET',
            url=url,
            headers=_as_str_dict(_headers),
            path_params={},  # _path always {}
            query_params=_as_str_dict(_query),
            body=None        # _body always None
        )
        resp = await self._client.execute(req)
        return resp
