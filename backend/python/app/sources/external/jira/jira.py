...
# Precomputed empty string-dict for fast path
_AS_STR_EMPTY_DICT: Dict[str, str] = {}
...
Body (application/json) fields:
  - selectedIssueIdsOrKeys (list[str], required)"""
        ...
        # Avoid dict copy when headers is None/mutable constructor only when headers given
        if headers:
        ...
        # Path and query params are always empty in this method, so their string conversion is statically known
        _path = _AS_STR_EMPTY_DICT
        _query = _AS_STR_EMPTY_DICT

        # Body always contains only the provided issues
        _body: Dict[str, Any] = {'selectedIssueIdsOrKeys': selectedIssueIdsOrKeys}

        # Optimization: rel_path is static, no formatting required as _path is always empty
