...
Body (application/json) fields:
  ...
  - workflowsForIssueTypes (list[Dict[str, Any]], required)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Fast-path: directly copy headers if passed in, else start with empty
        ...
        url = self.base_url + _safe_format_url(rel_path, {})  # _path always empty

        req = HTTPRequest(
            method='POST',
            url=url,
            headers=_as_str_dict(_headers),
            path_params={},  # pass literal empty dict
            query_params={},  # pass literal empty dict
            body=_body
        )
        # Await response
        resp = await self._client.execute(req)
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Early fast-path for empty dicts; avoid comprehension entirely
    if not d:
        return {}
