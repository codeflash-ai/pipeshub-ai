...
Body (application/json) fields:
  ...
  - validationOptions (Dict[str, Any], optional)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Avoid unnecessary dict copy/allocation if headers is None/empty
        if headers:
            _headers: Dict[str, Any] = dict(headers)
            _headers.setdefault('Content-Type', 'application/json')
        else:
            _headers = {'Content-Type': 'application/json'}
        # Use shared immutable empty dicts for path/query; endpoint never uses them
        _path = _EMPTY_DICT
        _query = _EMPTY_DICT
        ...
        # Fast path: No actual url formatting since _path is always empty, avoid format call
