...
Query params:
  - expand (str, optional)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        _headers = headers if headers is not None else {}
        # The 'id' is always str, so no need to serialize. 
        # This removes redundant call to str() and _serialize_value().
        _path = {'id': id}
        # Only create _query if expand is provided
        if expand is not None:
            _query = {'expand': expand}
        else:
            _query = None  # Avoid unnecessary empty dict
        _body = None
        rel_path = '/rest/api/3/version/{id}'
        url = self.base_url + _fast_safe_format_url(rel_path, _path)
        # Pre-serialize headers and path (header keys/values and id are always str)
        req_headers = _headers if not _headers else _as_str_dict(_headers)
        req_path = _path  # id is always str, so directly use
        req_query = _query if not _query else _as_str_dict(_query)
        req = HTTPRequest(
            method='GET',
            url=url,
            headers=req_headers,
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Optimize for dicts where items are already str
    out = {}
    for k, v in d.items():
        # inline check for str, avoids unnecessary function call
        if isinstance(v, str):
            out[str(k)] = v
        else:
...
# Fast-path: Avoid _SafeDict overhead when all path values used safely
def _fast_safe_format_url(template: str, params: Dict[str, object]) -> str:
    try:
        # For single {id} key, avoid constructing _SafeDict at each call
