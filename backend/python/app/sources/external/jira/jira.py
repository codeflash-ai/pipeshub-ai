...
Body (application/json) fields:
  - issueIdsOrKeys (list[str], optional)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Use provided headers directly if given, else make a new dict; ensures less dictionary copying and mutation.
        ...
        if issueIdsOrKeys is not None:
            _body['issueIdsOrKeys'] = issueIdsOrKeys
        rel_path = '/rest/api/3/issue/archive'
        url = self.base_url + _safe_format_url(rel_path, {})  # No path params for this endpoint
        req = HTTPRequest(
            method='PUT',
            url=url,
            headers=_as_str_dict(_headers),
            path_params={},  # avoid _as_str_dict({}) allocation for empty
            query_params={}, # avoid _as_str_dict({}) allocation for empty
            body=_body,
        )
        resp = await self._client.execute(req)
        return resp
