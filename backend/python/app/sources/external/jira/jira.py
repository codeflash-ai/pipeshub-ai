...
Query params:
  ...
  - id (list[int], optional)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Use dict(headers) only if headers is not None and not already a dict
        _headers = headers if headers is not None else {}
        # Build _query efficiently with dict comprehension
        _query = {}
        if startAt is not None:
            _query['startAt'] = startAt
        if maxResults is not None:
        ...
        # Avoid unnecessary dict conversion for _path since it's always empty
        url = self.base_url + _safe_format_url(rel_path, {})
        # Only call _as_str_dict if dict is non-empty
        headers_str = _as_str_dict(_headers) if _headers else {}
        req = HTTPRequest(method='GET', url=url, headers=headers_str, path_params={}, query_params=_as_str_dict(_query) if _query else {}, body=None)
        resp = await self._client.execute(req)
        return resp
...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    if params:  # Avoid function call overhead if params is empty
        try:
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Use generator inside dict constructor for slight memory efficiency
