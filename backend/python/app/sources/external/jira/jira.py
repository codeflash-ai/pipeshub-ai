...
Body (application/json) fields:
  ...
  - sharePermissions (list[Dict[str, Any]], required)"""
        ...
        # Efficient headers construction (no re-conversion needed if already dict[str, Any])
        ...
        # Body: batch dict update, avoids multiple lines
        _body = {
        ...
        rel_path = '/rest/api/3/dashboard/{id}'
        url = self.base_url + _safe_format_url(rel_path, _path)
        # Use local _serialize_value for performance
        _serialize_value_ref = _serialize_value
        # Skip _as_str_dict for _headers/_path if only strings
        req = HTTPRequest(
            method='PUT',
            url=url,
            headers={str(k): _serialize_value_ref(v) for k, v in _headers.items()},
            path_params={'id': str(id)},
            query_params={str(k): _serialize_value_ref(v) for k, v in _query.items()},
            body=_body
        )
        resp = await self._client.execute(req)
        return resp
...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    try:
        # Use SafeDict inline if needed elsewhere, assume standard dict suffices for format_map
