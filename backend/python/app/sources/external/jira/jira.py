...
# Reusable immutable empty dicts for micro-optimization
...
HTTP GET /rest/api/3/issueLinkType"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        _headers = dict(headers) if headers else EMPTY_DICT
        # Use preallocated EMPTY_DICT for _path and _query since these are always empty here
        _path = EMPTY_DICT
        _query = EMPTY_DICT
        _body = None
        # rel_path is always static, moved to constant
        url = self.base_url + _safe_format_url(REL_PATH, _path)
        req = HTTPRequest(
            method='GET',
            url=url,
...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    # Fast-path: if params is empty, skip format_map for performance
    if not params:
        return template
    # Use closure to store the class reference
    class_ref = _SafeDict
    try:
        return template.format_map(class_ref(params))
    except Exception:
        return template
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Fast-path: if dict is empty, avoid loop/function call
    if not d:
        return {}
    sv = _serialize_value  # Local variable for micro-optimization
    out = {}
    for k, v in d.items():
        out[str(k)] = sv(v)
    return out
