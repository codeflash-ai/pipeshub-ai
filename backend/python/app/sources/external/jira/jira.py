...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Fast-path: avoid allocation if empty
    if not d:
        return {}
    # Optimize for common case: all keys and values are str or None
    result = {}
    for k, v in d.items():
        ...
        if isinstance(v, str):
            ...
            result[sk] = 'true' if v else 'false'
        elif isinstance(v, (int, float)):
            result[sk] = str(v)
        elif isinstance(v, (list, tuple, set)):
            result[sk] = ','.join('true' if x is True else 'false' if x is False else '' if x is None else str(x) for x in v)
        else:
            result[sk] = str(v)
    return result
