...
# Preallocated immutable objects to avoid re-creations in hot path
...
Path params:
  - linkId (str)"""
        ...
        _path = {'linkId': linkId}
        # Use preallocated empty dict and str dict for _query and _body instead of creating every call
        _query = _EMPTY_DICT
        _body = None
        rel_path = '/rest/api/3/issueLink/{linkId}'
        url = self.base_url + _safe_format_url(rel_path, _path)
        # Avoid redundant conversion for known empty dict cases, else convert only as needed
        req = HTTPRequest(
            method='DELETE',
            url=url,
            headers=_as_str_dict(_headers) if _headers else _EMPTY_STR_DICT,
            path_params=_as_str_dict(_path),  # always 1 item
            query_params=_EMPTY_STR_DICT,     # always empty
            body=_body
        )
        resp = await self._client.execute(req)
        return resp
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Minor optimization: fast-path for empty dict
    if not d:
