...
Path params:
  ...
  - worklogId (str)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Inline comprehensions for efficiency
        _headers = {str(k): _serialize_value(v) for (k, v) in (headers or {}).items()}
        _path = {str(k): _serialize_value(v) for (k, v) in {'issueIdOrKey': issueIdOrKey, 'worklogId': worklogId}.items()}
        _query = {}
        _body = None
        rel_path = '/rest/api/3/issue/{issueIdOrKey}/worklog/{worklogId}/properties'
        url = self.base_url + _safe_format_url(rel_path, _path)
        req = HTTPRequest(method='GET', url=url, headers=_headers, path_params=_path, query_params=_query, body=_body)
        resp = await self._client.execute(req)
        return resp
