...
Path params:
  - id (int)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        _headers: Dict[str, Any] = headers or {}
        _path: Dict[str, Any] = {'id': id}
        _query: Dict[str, Any] = {}
        _body = None
        rel_path = '/rest/api/3/fieldconfigurationscheme/{id}'
        url = self.base_url + _safe_format_url(rel_path, _path)
        req = HTTPRequest(method='DELETE', url=url, headers=_as_str_dict(_headers) if _headers else {}, path_params=_as_str_dict(_path), query_params={}, body=_body)
        resp = await self._client.execute(req)
        return resp
