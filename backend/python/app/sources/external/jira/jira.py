...
Body (application/json) fields:
  - selectedIssueIdsOrKeys (list[str], required)"""
        ...
        # Merge headers efficiently and ensure Content-Type is set
        if headers:
        ...
        # Empty dicts reused, avoid allocation and conversion
        _path = {}
        _query = {}
        # Only one field, dict literal is fastest
        _body = {'selectedIssueIdsOrKeys': selectedIssueIdsOrKeys}

        rel_path = '/rest/api/3/bulk/issues/unwatch'

        # Avoid calling _safe_format_url for empty path params
        if not _path:
...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    # Fast path: if params is empty, skip formatting entirely
    if not params:
        return template
    try:
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    ...
    if not d:
        return {}
    # Fast path for single item dict for efficiency in tight loops
    if len(d) == 1:
