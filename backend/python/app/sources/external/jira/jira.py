...
Path params:
  - id (str)"""
        ...
        # Use str.format for path formatting directly if param keys are valid
        try:
            rel_url = rel_path.format(id=id)
        except Exception:
            rel_url = rel_path
        url = f"{self.base_url}{rel_url}"

        # Pre-serialize dicts in call site to avoid redundant dictionary construction
        req = HTTPRequest(
            method='GET',
            url=url,
