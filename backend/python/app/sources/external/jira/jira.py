...
Body (application/json) fields:
  - id (str, required)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Directly assign (and avoid unnecessary dict copying if headers is None)
        if headers:
        ...
        # Avoid wasteful allocation/conversion for empty path/query dicts
        rel_path = '/rest/api/3/priority/default'
        url = self.base_url + rel_path

        _body: Dict[str, Any] = {'id': id}
