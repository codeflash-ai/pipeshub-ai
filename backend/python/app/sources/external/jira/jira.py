...
Query params:
  - accountId (list[str], required)"""
        ...
        # Only convert headers to string if provided, avoid empty conversion
        _headers: Dict[str, Any] = headers if headers else None

        # Path params for this method are always empty, skip work
        _path: Dict[str, Any] = {}

        # Query params is always just accountId
        # Avoid dict creation by passing directly to _as_str_dict
        _query = {'accountId': accountId}
        _body = None
        rel_path = '/rest/api/3/user/email/bulk'
        url = self.base_url + _safe_format_url(rel_path, _path)

        # Minimize calls to _as_str_dict
        headers_str = _as_str_dict(_headers) if _headers else {}
        path_str = {}  # Path is empty dict, skip conversion
...
def _safe_format_url(template: str, params: Dict[str, object]) -> str:
    # Optimize: if params is empty, skip formatting
    if not params:
        return template
    try:
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Optimize: skip if d is None or empty
    if not d:
        return {}
