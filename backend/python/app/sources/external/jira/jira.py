...
Query params:
  - updateDraftIfNeeded (bool, optional)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Prepare headers using direct conversion
        _headers = dict(headers) if headers else {}
        # Create rel_path and path values
        rel_path = '/rest/api/3/workflowscheme/{id}/default'
        _path = {'id': id}
        # Inline query params building
        if updateDraftIfNeeded is not None:
        ...
        _body = None
        url = self.base_url + _safe_format_url(rel_path, _path)
        # Build HTTPRequest, inlining _as_str_dict for tiny dicts
        req = HTTPRequest(
            method='DELETE',
            url=url,
            headers={str(k): _serialize_value(v) for k, v in _headers.items()},
            path_params={'id': _serialize_value(id)},
            query_params={k: _serialize_value(v) for k, v in _query.items()},
            body=_body
        )
        resp = await self._client.execute(req)
        return resp
