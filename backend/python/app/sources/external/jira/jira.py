...
Query params:
  ...
  - applicationKey (str, optional)"""
        ...
        _headers: Dict[str, Any] = headers.copy() if headers else _EMPTY_DICT

        # _path is always empty, so reuse singleton object
        _path: Dict[str, Any] = _EMPTY_DICT

        # Compact _query construction using local dict update
        _query: Dict[str, Any] = {}
        if startAt is not None:
            _query['startAt'] = startAt
        if maxResults is not None:
        ...
        # Only call _safe_format_url if path keys exist
        url = self.base_url + (rel_path if not _path else _safe_format_url(rel_path, _path))

        # Avoid call if dicts are empty (hot path!)
        req = HTTPRequest(
            method='GET',
            url=url,
            headers=_as_str_dict(_headers) if _headers else _EMPTY_STR_DICT,
            path_params=_EMPTY_STR_DICT,  # always empty
            query_params=_as_str_dict(_query) if _query else _EMPTY_STR_DICT,
            body=_body
        )
        resp = await self._client.execute(req)
        return resp
