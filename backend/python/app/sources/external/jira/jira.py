...
_CT_JSON_STR_DICT = {'Content-Type': 'application/json'}
...
Body (application/json) fields:
  - ids (list[int], required)"""
        ...
        if headers is None:
            ...
            _str_headers = _CT_JSON_STR_DICT
        else:
            _headers = dict(headers)
            if 'Content-Type' not in _headers:
                _headers['Content-Type'] = 'application/json'
            _str_headers = _as_str_dict(_headers)
        # _path is always empty, avoid allocation and repr
        _query: Dict[str, Any] = {}
        if expand is not None:
            _query['expand'] = expand
            _str_query = _as_str_dict(_query)
        else:
            _str_query = _EMPTY_STR_DICT
        _body: Dict[str, Any] = {'ids': ids}
        rel_path = '/rest/api/3/comment/list'
        # Avoid needless formatting if no path params
...
def _to_bool_str(x):
    # bools become lowercase strings, rest str as normal
