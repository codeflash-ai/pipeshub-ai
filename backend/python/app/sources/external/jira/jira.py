...
# Static precomputed empty str dict
...
Body: application/json (str)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Avoid constructing new dict when not needed
        if headers:
        ...
        # _path keys are always str, so avoid str(k) conversion in _as_str_dict
        _path = {'addonKey': addonKey, 'propertyKey': propertyKey}
        # _query is always empty, so use static
        _query = EMPTY_STR_DICT
        _body = body
        rel_path = '/rest/atlassian-connect/1/addons/{addonKey}/properties/{propertyKey}'
        url = self.base_url + _safe_format_url(rel_path, _path)
        req = HTTPRequest(
            ...
            path_params=_path,  # Avoid unnecessary conversion; keys are already str
            query_params=_query,
            body=_body
        )
        resp = await self._client.execute(req)
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # Fast path if all keys are str and all values are primitives or None
    if not d:
        return EMPTY_STR_DICT
    out = {}
    # Avoid unnecessary str conversion for keys, since usually they're already str
    for k, v in d.items():
        if type(k) is str:
