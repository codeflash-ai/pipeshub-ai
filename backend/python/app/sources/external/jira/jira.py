...
Path params:
  - screenId (int)"""
        if self._client is None:
            raise ValueError('HTTP client is not initialized')
        # Inline path dictionary (only one parameter)
        _path = {'screenId': screenId}
        # Only convert headers/query if not empty
        _headers = headers if headers else None
        _query = None  # always empty
        _body = None
        rel_path = '/rest/api/3/screens/{screenId}'
        # Only call _as_str_dict once for _path, use result for both URL and request
        str_path = _as_str_dict(_path)
        url = self.base_url + _safe_format_url(rel_path, str_path)
        # Only convert headers if needed, avoid creating unnecessary dict objects for empty
        req = HTTPRequest(
            method='DELETE',
            url=url,
