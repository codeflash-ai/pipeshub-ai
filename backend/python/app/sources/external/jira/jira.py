...
_REL_PATH = '/rest/api/3/filter/defaultShareScope'
...
HTTP GET /rest/api/3/filter/defaultShareScope"""
        ...
        url = f"{self.base_url}{_safe_format_url(_REL_PATH, _EMPTY_DICT)}"
        req = HTTPRequest(
            method='GET',
            url=url,
            headers=_as_str_dict(_headers),
            path_params=_as_str_dict(_EMPTY_DICT),
            query_params=_as_str_dict(_EMPTY_DICT),
            body=_EMPTY_BODY
        )
        resp = await self._client.execute(req)
        return resp
...
def _as_str_dict(d: Dict[str, Any]) -> Dict[str, str]:
    # micro optimization: skip comprehension if already empty
    if not d:
        return {}
